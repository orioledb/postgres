...
int64 try_tableam_relation_size(Relation rel, ForkNumber forkNum, bool allforks, uint8 method)
{
	...
	elog(DEBUG3, "try_tableam_relation_size");
	if (rel->rd_rel->relkind == RELKIND_INDEX)
	{
		/* For index check rd_tableam for parent relation */
		Relation        tbl;

		elog(DEBUG3, "RELKIND_INDEX, OPEN PARENT");
		tbl = relation_open(rel->rd_index->indrelid, AccessShareLock);
		if(tbl->rd_tableam)
		{
			elog(DEBUG3, "PARENT HAS TABLE AM");
			if(tbl->rd_tableam->relation_size)
			{
				elog(DEBUG3, "PARENY HAS RELSIZE METHOD");
	...
	{
		elog(DEBUG3, "RELKIND_TABLE");
		elog(DEBUG3, "HAS TABLE AM");
		if(rel->rd_tableam->relation_size)
		{
			elog(DEBUG3, "HAS RELSIZE METHOD");
	...
	elog(DEBUG3, "FALLBACK TO calculate_relation_size");
	return -1;
}
