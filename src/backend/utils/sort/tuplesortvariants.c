						...
						SortCoordinate coordinate, bool randomAccess)
{
	...
	leading = arg->indexInfo->ii_IndexAttrNumbers[0];
...
tuplesort_putheaptuple(Tuplesortstate *state, HeapTuple tup)
{
	...
	tuplesort_puttuple_common(state, &stup,
					arg->indexInfo->ii_IndexAttrNumbers[0] != 0 && base->sortKeys->abbrev_converter && !stup.isnull1);
...
static void
writetup_heap(Tuplesortstate *state, int tapenum, SortTupleCompat *stup)
{
	TuplesortPublic *base = TuplesortstateGetPublic(state);
	...
	LogicalTapeWrite(tapeset, tapenum,
					 (void *) &tuplen, sizeof(tuplen));
	LogicalTapeWrite(tapeset, tapenum,
					 (void *) tupbody, tupbodylen);
	if (base->randomAccess)	/* need trailing length word? */
		LogicalTapeWrite(tapeset, tapenum,
						 (void *) &tuplen, sizeof(tuplen));
}
			 ...
			 int tapenum, unsigned int len)
{
	...
	tuple->t_len = tuplen;
	LogicalTapeReadExact(tapeset, tapenum,
						 tupbody, tupbodylen);
	if (base->randomAccess)	/* need trailing length word? */
		LogicalTapeReadExact(tapeset, tapenum,
...
static void
writetup_cluster(Tuplesortstate *state, int tapenum, SortTupleCompat *stup)
{
	TuplesortPublic *base = TuplesortstateGetPublic(state);
...
static void
writetup_index(Tuplesortstate *state, int tapenum, SortTupleCompat *stup)
{
	TuplesortPublic *base = TuplesortstateGetPublic(state);
...
static void
writetup_datum(Tuplesortstate *state, int tapenum, SortTupleCompat *stup)
{
	TuplesortPublic *base = TuplesortstateGetPublic(state);
	TuplesortDatumArg *arg = (TuplesortDatumArg *) base->arg;
	LogicalTapeSet *tapeset = tuplesort_get_tapeset(state);
