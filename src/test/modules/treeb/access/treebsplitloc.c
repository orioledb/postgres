 ...
 *
 * treebsplitloc.c
 *	  Choose split point code for Postgres treeb implementation.
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/treeb/treebsplitloc.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
typedef struct
{
	...
	/* split point identifying fields (returned by _treeb_findsplitloc) */
...
typedef struct
{
	/* context data for _treeb_recsplitloc */
...
static void _treeb_recsplitloc(FindSplitData *state,
							OffsetNumber firstrightoff, bool newitemonleft,
							int olddataitemstoleft,
							Size firstrightofforigpagetuplesz);
static void _treeb_deltasortsplits(FindSplitData *state, double fillfactormult,
								bool usemult);
static int	_treeb_splitcmp(const void *arg1, const void *arg2);
static bool _treeb_afternewitemoff(FindSplitData *state, OffsetNumber maxoff,
								int leaffillfactor, bool *usemult);
static bool _treeb_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid);
static OffsetNumber _treeb_bestsplitloc(FindSplitData *state, int perfectpenalty,
									 bool *newitemonleft, FindSplitStrat strategy);
static int	_treeb_defaultinterval(FindSplitData *state);
static int	_treeb_strategy(FindSplitData *state, SplitPoint *leftpage,
						 SplitPoint *rightpage, FindSplitStrat *strategy);
static void _treeb_interval_edges(FindSplitData *state,
							   SplitPoint **leftinterval, SplitPoint **rightinterval);
static inline int _treeb_split_penalty(FindSplitData *state, SplitPoint *split);
static inline IndexTuple _treeb_split_lastleft(FindSplitData *state,
											SplitPoint *split);
static inline IndexTuple _treeb_split_firstright(FindSplitData *state,
											  SplitPoint *split);


/*
 *	_treeb_findsplitloc() -- find an appropriate place to split a page.
 *
 ...
 * This is the same as treebsort.c produces for a newly-created tree.  Note
 ...
 * (!heapkeyspace indexes).  See treeb/README for more information about
 * suffix truncation.
 *
 ...
 */
OffsetNumber
_treeb_findsplitloc(Relation rel,
				 Page origpage,
				 ...
				 bool *newitemonleft)
{
	...
	opaque = TREEBPageGetOpaque(origpage);
	maxoff = PageGetMaxOffsetNumber(origpage);

	/* Total free space available on a treeb page, after fixed overhead */
	...
	leaffillfactor = TreebGetFillFactor(rel);
	...
	Assert(!TreebTupleIsPosting(newitem));
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		 ...
		 * page. (_treeb_recsplitloc() will reject the split when there are no
		 * previous items, which we rely on.)
		 */
		if (offnum < newitemoff)
			_treeb_recsplitloc(&state, offnum, false, olddataitemstoleft, itemsz);
		else if (offnum > newitemoff)
			_treeb_recsplitloc(&state, offnum, true, olddataitemstoleft, itemsz);
		else
		{
			/*
	...
	if (newitemoff > maxoff)
		_treeb_recsplitloc(&state, newitemoff, false, olddataitemstotal, 0);
	 ...
	 * considered.  See _treeb_defaultinterval().
	 */
	if (!state.is_leaf)
	{
		/* fillfactormult only used on rightmost page */
		usemult = state.is_rightmost;
		fillfactormult = TREEB_NONLEAF_FILLFACTOR / 100.0;
	}
	...
	else if (_treeb_afternewitemoff(&state, maxoff, leaffillfactor, &usemult))
	{
		/*
	...
	_treeb_deltasortsplits(&state, fillfactormult, usemult);

	/* Determine split interval for default strategy */
	state.interval = _treeb_defaultinterval(&state);
	 ...
	 */
	perfectpenalty = _treeb_strategy(&state, &leftpage, &rightpage, &strategy);
	...
	else if (strategy == SPLIT_SINGLE_VALUE)
	{
		...
		usemult = true;
		fillfactormult = TREEB_SINGLEVAL_FILLFACTOR / 100.0;
	 ...
	 */
	firstrightoff = _treeb_bestsplitloc(&state, perfectpenalty, newitemonleft,
									 strategy);
	pfree(state.splits);
 ...
 */
static void
_treeb_recsplitloc(FindSplitData *state,
				...
				Size firstrightofforigpagetuplesz)
{
	...
	else
	{
		...
		if (state->is_leaf && firstrightsz > 64)
		{
			...
			if (TreebTupleIsPosting(newhighkey))
				postingsz = IndexTupleSize(newhighkey) -
					TreebTupleGetPostingOffset(newhighkey);
		}
	}
 ...
 */
static void
_treeb_deltasortsplits(FindSplitData *state, double fillfactormult,
					bool usemult)
{
	for (int i = 0; i < state->nsplits; i++)
	{
	...
	qsort(state->splits, state->nsplits, sizeof(SplitPoint), _treeb_splitcmp);
}

/*
 * qsort-style comparator used by _treeb_deltasortsplits()
 */
static int
_treeb_splitcmp(const void *arg1, const void *arg2)
{
 ...
 */
static bool
_treeb_afternewitemoff(FindSplitData *state, OffsetNumber maxoff,
					int leaffillfactor, bool *usemult)
{
	int16		nkeyatts;
	ItemId		itemid;
	...
	if (state->newitemoff > maxoff)
	{
		...
		keepnatts = _treeb_keep_natts_fast(state->rel, tup, state->newitem);
	...
	if (TreebTupleIsPosting(tup) ||
		!_treeb_adjacenthtid(&tup->t_tid, &state->newitem->t_tid))
		return false;
 ...
 */
static bool
_treeb_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid)
{
	BlockNumber lowblk,
				highblk;
 ...
 * splitting a leaf page or an internal page.  See _treeb_split_penalty() for
 * details.
 *
 ...
 */
static OffsetNumber
_treeb_bestsplitloc(FindSplitData *state, int perfectpenalty,
				 bool *newitemonleft, FindSplitStrat strategy)
{
	int			bestpenalty,
				lowsplit;
	...
	for (int i = lowsplit; i < highsplit; i++)
	{
		...
		penalty = _treeb_split_penalty(state, state->splits + i);
 ...
 * interval is applied within _treeb_bestsplitloc().
 *
 ...
 * which case the interval also changes -- see _treeb_strategy()).
 *
 ...
 */
static int
_treeb_defaultinterval(FindSplitData *state)
{
 ...
 * passed to _treeb_bestsplitloc() as a final constraint on how far caller is
 * willing to go to avoid appending a heap TID when using the many duplicates
 * strategy (it also saves _treeb_bestsplitloc() useless cycles).
 */
static int
_treeb_strategy(FindSplitData *state, SplitPoint *leftpage,
			 SplitPoint *rightpage, FindSplitStrat *strategy)
{
	IndexTuple	leftmost,
				rightmost;
	...
	*strategy = SPLIT_DEFAULT;
	 ...
	 */
	_treeb_interval_edges(state, &leftinterval, &rightinterval);
	leftmost = _treeb_split_lastleft(state, leftinterval);
	rightmost = _treeb_split_firstright(state, rightinterval);
	 ...
	 */
	perfectpenalty = _treeb_keep_natts_fast(state->rel, leftmost, rightmost);
	 ...
	 */
	leftmost = _treeb_split_lastleft(state, leftpage);
	rightmost = _treeb_split_firstright(state, rightpage);
	...
	if (perfectpenalty <= indnkeyatts)
	{
		*strategy = SPLIT_MANY_DUPLICATES;
		 ...
		 * before a final decision is made, within _treeb_bestsplitloc().
		 */
		return indnkeyatts;
	}
	...
	else
	{
		...
		perfectpenalty = _treeb_keep_natts_fast(state->rel, hikey,
											 state->newitem);
		if (perfectpenalty <= indnkeyatts)
			*strategy = SPLIT_SINGLE_VALUE;
		else
 ...
 */
static void
_treeb_interval_edges(FindSplitData *state, SplitPoint **leftinterval,
 ...
 */
static inline int
_treeb_split_penalty(FindSplitData *state, SplitPoint *split)
{
	IndexTuple	lastleft;
	...
	lastleft = _treeb_split_lastleft(state, split);
	firstright = _treeb_split_firstright(state, split);

	return _treeb_keep_natts_fast(state->rel, lastleft, firstright);
}
...
_treeb_split_lastleft(FindSplitData *state, SplitPoint *split)
{
	ItemId		itemid;
 ...
 */
static inline IndexTuple
_treeb_split_firstright(FindSplitData *state, SplitPoint *split)
{
	ItemId		itemid;
