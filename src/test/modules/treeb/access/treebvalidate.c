 ...
 *
 * treebvalidate.c
 *	  Opclass validator for treeb.
 *
 * Portions Copyright (c) 1996-2024, PostgreSQL Global Development Group
 * Portions Copyright (c) 1994, Regents of the University of California
 *
 * IDENTIFICATION
 *	  src/backend/access/treeb/treebvalidate.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
treebvalidate(Oid opclassoid)
{
		...
		switch (procform->amprocnum)
		{
			...
			case TREEBINRANGE_PROC:
				ok = check_amproc_signature(procform->amproc, BOOLOID, true,
											...
											BOOLOID, BOOLOID);
				break;
			case TreebEQUALIMAGE_PROC:
				ok = check_amproc_signature(procform->amproc, BOOLOID, true,
											1, 1, OIDOID);
				break;
			case TREEBOPTIONS_PROC:
	...
	for (i = 0; i < oprlist->n_members; i++)
	{
		...
		if (oprform->amopstrategy < 1 ||
			oprform->amopstrategy > TreebMaxStrategyNumber)
		{
			ereport(INFO,
					(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
		...
		/* treeb doesn't support ORDER BY operators */
		...
		/* Check operator signature --- same for all treeb strategies */
	...
	foreach(lc, grouplist)
	{
		...
		if (thisgroup->operatorset == 0 &&
			thisgroup->functionset == (1 << TREEBINRANGE_PROC))
			continue;
		...
		if (thisgroup->operatorset !=
			((1 << TreebLessStrategyNumber) |
			 (1 << TreebLessEqualStrategyNumber) |
			 (1 << TreebEqualStrategyNumber) |
			 (1 << TreebGreaterEqualStrategyNumber) |
			 (1 << TreebGreaterStrategyNumber)))
		{
			ereport(INFO,
					(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
		...
		if ((thisgroup->functionset & (1 << TREEBORDER_PROC)) == 0)
		{
			ereport(INFO,
					(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
	 ...
	 * reasonable to insist that all built-in treeb opfamilies be complete.
	 */
...
/*
 * Prechecking function for adding operators/functions to a treeb opfamily.
 */
void
treebadjustmembers(Oid opfamilyoid,
				Oid opclassoid,
				List *operators,
				List *functions)
{
	...
	/*
	 * Treeb operators and comparison support functions are always "loose"
	...
	foreach(lc, list_concat_copy(operators, functions))
	{
		...
		if (op->is_func && op->number != TREEBORDER_PROC)
		{
		...
		else
		{
			...
			if (op->lefttype != opcintype)
			{
				...
				opclassoid = opclass_for_family_datatype(GetTreebOid(),
														 opfamilyoid,
														 opcintype);
			}
			if (OidIsValid(opclassoid))
