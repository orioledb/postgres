 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/treeb/treebxlog.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
/*
 * _treeb_restore_page -- re-enter all the index tuples on a page
 *
 ...
 */
static void
_treeb_restore_page(Page page, char *from, int len)
{
	...
	for (i = nitems - 1; i >= 0; i--)
	{
		if (PageAddItem(page, items[i], itemsizes[i], nitems - i,
			...
			elog(PANIC, "_treeb_restore_page: cannot add item to page");
	}
}

static void
_treeb_restore_meta(XLogReaderState *record, uint8 block_id)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	Buffer		metabuf;
	Page		metapg;
	TreebMetaPageData *md;
	TreebPageOpaque pageop;
	...
	Assert(len == sizeof(xl_treeb_metadata));
	Assert(BufferGetBlockNumber(metabuf) == TREEB_METAPAGE);
	xlrec = (xl_treeb_metadata *) ptr;
	metapg = BufferGetPage(metabuf);

	_treeb_pageinit(metapg, BufferGetPageSize(metabuf));

	md = TreebPageGetMeta(metapg);
	md->treebm_magic = TREEB_MAGIC;
	md->treebm_version = xlrec->version;
	md->treebm_root = xlrec->root;
	md->treebm_level = xlrec->level;
	md->treebm_fastroot = xlrec->fastroot;
	md->treebm_fastlevel = xlrec->fastlevel;
	/* Cannot log TREEB_MIN_VERSION index metapage without upgrade */
	Assert(md->treebm_version >= TREEB_NOVAC_VERSION);
	md->treebm_last_cleanup_num_delpages = xlrec->last_cleanup_num_delpages;
	md->treebm_last_cleanup_num_heap_tuples = -1.0;
	md->treebm_allequalimage = xlrec->allequalimage;

	pageop = TREEBPageGetOpaque(metapg);
	pageop->treebpo_flags = TREEBP_META;
	...
	((PageHeader) metapg)->pd_lower =
		((char *) md + sizeof(TreebMetaPageData)) - (char *) metapg;
...
/*
 * _treeb_clear_incomplete_split -- clear INCOMPLETE_SPLIT flag on a page
 *
 * This is a common subroutine of the redo functions of all the WAL record
 * types that can insert a downlink: insert, split, and newroot.
 */
static void
_treeb_clear_incomplete_split(XLogReaderState *record, uint8 block_id)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	Buffer		buf;
	...
	if (XLogReadBufferForRedo(record, block_id, &buf) == BLK_NEEDS_REDO)
	{
		...
		TreebPageOpaque pageop = TREEBPageGetOpaque(page);

		Assert(P_INCOMPLETE_SPLIT(pageop));
		pageop->treebpo_flags &= ~TREEBP_INCOMPLETE_SPLIT;
...
static void
treeb_xlog_insert(bool isleaf, bool ismeta, bool posting,
				  XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_insert *xlrec = (xl_treeb_insert *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	 ...
	 */
	if (!isleaf)
		_treeb_clear_incomplete_split(record, 1);
	if (XLogReadBufferForRedo(record, 0, &buffer) == BLK_NEEDS_REDO)
	{
		...
		else
		{
			 ...
			 *
			 * Use _treeb_swap_posting() to repeat posting list split steps from
			...
			/* Use mutable, aligned newitem copy in _treeb_swap_posting() */
			Assert(isleaf && postingoff > 0);
			newitem = CopyIndexTuple((IndexTuple) datapos);
			nposting = _treeb_swap_posting(newitem, oposting, postingoff);
	 ...
	 */
	if (ismeta)
		_treeb_restore_meta(record, 2);
}

static void
treeb_xlog_split(bool newitemonleft, XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_split *xlrec = (xl_treeb_split *) XLogRecGetData(record);
	 ...
	 * internal page split).  This is like the corresponding treeb_xlog_insert
	 ...
	 * passing).
	 *
	 * Like in treeb_xlog_insert, this can be done before locking other pages.
	 * We never need to couple cross-level locks in REDO routines.
	 */
	if (!isleaf)
		_treeb_clear_incomplete_split(record, 3);
	...
	_treeb_pageinit(rpage, BufferGetPageSize(rbuf));
	ropaque = TREEBPageGetOpaque(rpage);

	ropaque->treebpo_prev = origpagenumber;
	ropaque->treebpo_next = spagenumber;
	ropaque->treebpo_level = xlrec->level;
	ropaque->treebpo_flags = isleaf ? TREEBP_LEAF : 0;
	ropaque->treebpo_cycleid = 0;

	_treeb_restore_page(rpage, datapos, datalen);
	...
	if (XLogReadBufferForRedo(record, 0, &buf) == BLK_NEEDS_REDO)
	{
		 ...
		 * items to that in item number order.  This mirrors how _treeb_split()
		 * works.  Retaining the same physical order makes WAL consistency
		 * checking possible.  See also _treeb_restore_page(), which does the
		 * same for the right page.
		 */
		Page		origpage = (Page) BufferGetPage(buf);
		TreebPageOpaque oopaque = TREEBPageGetOpaque(origpage);
		OffsetNumber off;
		...
		if (newitemonleft || xlrec->postingoff != 0)
		{
			...
			if (xlrec->postingoff != 0)
			{
				...
				nposting = _treeb_swap_posting(newitem, oposting,
											xlrec->postingoff);
			}
		}
		...
		/* Fix opaque fields */
		oopaque->treebpo_flags = TREEBP_INCOMPLETE_SPLIT;
		if (isleaf)
			oopaque->treebpo_flags |= TREEBP_LEAF;
		oopaque->treebpo_next = rightpagenumber;
		oopaque->treebpo_cycleid = 0;
	...
	if (spagenumber != P_NONE)
	{
		...
		if (XLogReadBufferForRedo(record, 2, &sbuf) == BLK_NEEDS_REDO)
		{
			...
			TreebPageOpaque spageop = TREEBPageGetOpaque(spage);

			spageop->treebpo_prev = rightpagenumber;
...
static void
treeb_xlog_dedup(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_dedup *xlrec = (xl_treeb_dedup *) XLogRecGetData(record);
	Buffer		buf;
	...
	if (XLogReadBufferForRedo(record, 0, &buf) == BLK_NEEDS_REDO)
	{
		...
		OffsetNumber offnum,
					minoff,
					maxoff;
		TreebDedupState state;
		TreebDedupInterval *intervals;
		Page		newpage;

		state = (TreebDedupState) palloc(sizeof(TreebDedupStateData));
		state->deduplicate = true;	/* unused */
		state->nmaxitems = 0;	/* unused */
		/* Conservatively use larger maxpostingsize than primary */
		state->maxpostingsize = TreebMaxItemSize(page);
		state->base = NULL;
		...
		intervals = (TreebDedupInterval *) ptr;
		for (offnum = minoff;
			 offnum <= maxoff;
			 offnum = OffsetNumberNext(offnum))
		{
			...
			if (offnum == minoff)
				_treeb_dedup_start_pending(state, itup, offnum);
			...
			{
				if (!_treeb_dedup_save_htid(state, itup))
					elog(ERROR, "deduplication failed to add heap tid to pending posting list");
			}
			else
			{
				_treeb_dedup_finish_pending(newpage, state);
				_treeb_dedup_start_pending(state, itup, offnum);
			}
		}
		...
		Assert(memcmp(state->intervals, intervals,
					  state->nintervals * sizeof(TreebDedupInterval)) == 0);

		if (P_HAS_GARBAGE(opaque))
		{
			TreebPageOpaque nopaque = TREEBPageGetOpaque(newpage);

			nopaque->treebpo_flags &= ~TREEBP_HAS_GARBAGE;
		}
...
static void
treeb_xlog_updates(Page page, OffsetNumber *updatedoffsets,
				   xl_treeb_update *updates, int nupdated)
{
	...
	for (int i = 0; i < nupdated; i++)
	{
		...
		/* advance to next xl_treeb_update from array */
...
static void
treeb_xlog_vacuum(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_vacuum *xlrec = (xl_treeb_vacuum *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	TreebPageOpaque opaque;

	/*
	 * We need to take a cleanup lock here, just like treebvacuumpage(). However,
	 ...
	 * items to kill suffices).  See treeb/README for details.
	 */
		...
		== BLK_NEEDS_REDO)
	{
		...
		if (xlrec->nupdated > 0)
		{
			...
			treeb_xlog_updates(page, updatedoffsets, updates, xlrec->nupdated);
		}
...
static void
treeb_xlog_delete(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_delete *xlrec = (xl_treeb_delete *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	TreebPageOpaque opaque;
...
static void
treeb_xlog_mark_page_halfdead(uint8 info, XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_mark_page_halfdead *xlrec = (xl_treeb_mark_page_halfdead *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	TreebPageOpaque pageop;
	...
	if (XLogReadBufferForRedo(record, 1, &buffer) == BLK_NEEDS_REDO)
	{
		...
		page = (Page) BufferGetPage(buffer);
		pageop = TREEBPageGetOpaque(page);
		...
		rightsib = TreebTupleGetDownLink(itup);

		itemid = PageGetItemId(page, poffset);
		itup = (IndexTuple) PageGetItem(page, itemid);
		TreebTupleSetDownLink(itup, rightsib);
		nextoffset = OffsetNumberNext(poffset);
	...
	_treeb_pageinit(page, BufferGetPageSize(buffer));
	pageop = TREEBPageGetOpaque(page);

	pageop->treebpo_prev = xlrec->leftblk;
	pageop->treebpo_next = xlrec->rightblk;
	pageop->treebpo_level = 0;
	pageop->treebpo_flags = TREEBP_HALF_DEAD | TREEBP_LEAF;
	pageop->treebpo_cycleid = 0;
	...
	TreebTupleSetTopParent(&trunctuple, xlrec->topparent);
...
static void
treeb_xlog_unlink_page(uint8 info, XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_unlink_page *xlrec = (xl_treeb_unlink_page *) XLogRecGetData(record);
	BlockNumber leftsib;
	BlockNumber rightsib;
	uint32		level;
	bool		isleaf;
	...
	_treeb_pageinit(page, BufferGetPageSize(target));
	pageop = TREEBPageGetOpaque(page);

	pageop->treebpo_prev = leftsib;
	pageop->treebpo_next = rightsib;
	pageop->treebpo_level = level;
	TreebPageSetDeleted(page, safexid);
	if (isleaf)
		pageop->treebpo_flags |= TREEBP_LEAF;
	pageop->treebpo_cycleid = 0;
	...
	if (XLogRecHasBlockRef(record, 3))
	{
		...
		_treeb_pageinit(page, BufferGetPageSize(leafbuf));
		pageop = TREEBPageGetOpaque(page);

		pageop->treebpo_flags = TREEBP_HALF_DEAD | TREEBP_LEAF;
		pageop->treebpo_prev = xlrec->leafleftsib;
		pageop->treebpo_next = xlrec->leafrightsib;
		pageop->treebpo_level = 0;
		pageop->treebpo_cycleid = 0;
		...
		MemSet(&trunctuple, 0, sizeof(IndexTupleData));
		trunctuple.t_info = sizeof(IndexTupleData);
		TreebTupleSetTopParent(&trunctuple, xlrec->leaftopparent);
	...
	if (info == XLOG_TREEB_UNLINK_PAGE_META)
		_treeb_restore_meta(record, 4);
}

static void
treeb_xlog_newroot(XLogReaderState *record)
{
	XLogRecPtr	lsn = record->EndRecPtr;
	xl_treeb_newroot *xlrec = (xl_treeb_newroot *) XLogRecGetData(record);
	Buffer		buffer;
	Page		page;
	TreebPageOpaque pageop;
	char	   *ptr;
	...
	pageop->treebpo_flags = TREEBP_ROOT;
	pageop->treebpo_prev = pageop->treebpo_next = P_NONE;
	pageop->treebpo_level = xlrec->level;
	if (xlrec->level == 0)
		pageop->treebpo_flags |= TREEBP_LEAF;
	pageop->treebpo_cycleid = 0;
	...
	if (xlrec->level > 0)
	{
		...
		_treeb_restore_page(page, ptr, len);
 ...
 * VACUUM earlier into the FSM now.  See treeb/README.
 *
 ...
 * _treeb_getbuf() verifies it's safe using TreebPageIsRecyclable(), but that's just
 ...
 * xl_treeb_reuse_page record at the point that a page is actually recycled
 * and reused for an entirely unrelated page inside _treeb_split().  These
 ...
 *
 * The GlobalVisCheckRemovableFullXid() test in TreebPageIsRecyclable() is used
 ...
 */
static void
treeb_xlog_reuse_page(XLogReaderState *record)
{
	xl_treeb_reuse_page *xlrec = (xl_treeb_reuse_page *) XLogRecGetData(record);
...
void
treeb_redo(XLogReaderState *record)
{
	...
	switch (info)
	{
		case XLOG_TREEB_INSERT_LEAF:
			treeb_xlog_insert(true, false, false, record);
			break;
		case XLOG_TREEB_INSERT_UPPER:
			treeb_xlog_insert(false, false, false, record);
			break;
		case XLOG_TREEB_INSERT_META:
			treeb_xlog_insert(false, true, false, record);
			break;
		case XLOG_TREEB_SPLIT_L:
			treeb_xlog_split(true, record);
			break;
		case XLOG_TREEB_SPLIT_R:
			treeb_xlog_split(false, record);
			break;
		case XLOG_TREEB_INSERT_POST:
			treeb_xlog_insert(true, false, true, record);
			break;
		case XLOG_TREEB_DEDUP:
			treeb_xlog_dedup(record);
			break;
		case XLOG_TREEB_VACUUM:
			treeb_xlog_vacuum(record);
			break;
		case XLOG_TREEB_DELETE:
			treeb_xlog_delete(record);
			break;
		case XLOG_TREEB_MARK_PAGE_HALFDEAD:
			treeb_xlog_mark_page_halfdead(info, record);
			break;
		case XLOG_TREEB_UNLINK_PAGE:
		case XLOG_TREEB_UNLINK_PAGE_META:
			treeb_xlog_unlink_page(info, record);
			break;
		case XLOG_TREEB_NEWROOT:
			treeb_xlog_newroot(record);
			break;
		case XLOG_TREEB_REUSE_PAGE:
			treeb_xlog_reuse_page(record);
			break;
		case XLOG_TREEB_META_CLEANUP:
			_treeb_restore_meta(record, 0);
			break;
		default:
			elog(PANIC, "treeb_redo: unknown op code %u", info);
	}
...
void
treeb_xlog_startup(void)
{
	opCtx = AllocSetContextCreate(CurrentMemoryContext,
								  "Treeb recovery temporary context",
								  ALLOCSET_DEFAULT_SIZES);
}

void
treeb_xlog_cleanup(void)
{
...
/*
 * Mask a treeb page before performing consistency checks on it.
 */
void
treeb_mask(char *pagedata, BlockNumber blkno)
{
	Page		page = (Page) pagedata;
	TreebPageOpaque maskopaq;
	...
	maskopaq = TREEBPageGetOpaque(page);

	if (P_ISLEAF(maskopaq))
	{
		/*
		 * In treeb leaf pages, it is possible to modify the LP_FLAGS without
		 * emitting any WAL record. Hence, mask the line pointer flags. See
		 * _treeb_killitems(), _treeb_check_unique() for details.
		 */
		mask_lp_flags(page);
	}

	/*
	 * TREEBP_HAS_GARBAGE is just an un-logged hint bit. So, mask it. See
	 * _treeb_delete_or_dedup_one_page(), _treeb_killitems(), and _treeb_check_unique()
	 * for details.
	 */
	maskopaq->treebpo_flags &= ~TREEBP_HAS_GARBAGE;

	/*
	 * During replay of a treeb page split, we don't set the TREEBP_SPLIT_END
	 * flag of the right sibling and initialize the cycle_id to 0 for the same
	 * page. See treeb_xlog_split() for details.
	 */
	maskopaq->treebpo_flags &= ~TREEBP_SPLIT_END;
	maskopaq->treebpo_cycleid = 0;
}
