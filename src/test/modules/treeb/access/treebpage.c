 ...
 *
 * treebpage.c
 *	  Treeb-specific page management code for the Postgres treeb access
 *	  method.
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/treeb/treebpage.c
 *
 *	NOTES
 *	   Postgres treeb pages look like ordinary relation pages.  The opaque
 *	   data at high addresses includes pointers to left and right siblings
 *	   and flag data describing page state.  The first page in a treeb, page
...
static TreebMetaPageData *_treeb_getmeta(Relation rel, Buffer metabuf);
static void _treeb_delitems_delete(Relation rel, Buffer buf,
								TransactionId snapshotConflictHorizon,
								bool isCatalogRel,
								OffsetNumber *deletable, int ndeletable,
								TreebVacuumPosting *updatable, int nupdatable);
static char *_treeb_delitems_update(TreebVacuumPosting *updatable, int nupdatable,
								 OffsetNumber *updatedoffsets,
								 Size *updatedbuflen, bool needswal);
static bool _treeb_mark_page_halfdead(Relation rel, Relation heaprel,
								   Buffer leafbuf, TreebStack stack);
static bool _treeb_unlink_halfdead_page(Relation rel, Buffer leafbuf,
									 BlockNumber scanblkno,
									 bool *rightsib_empty,
									 TreebVacState *vstate);
static bool _treeb_lock_subtree_parent(Relation rel, Relation heaprel,
									BlockNumber child, TreebStack stack,
									Buffer *subtreeparent, OffsetNumber *poffset,
									BlockNumber *topparent,
									BlockNumber *topparentrightsib);
static void _treeb_pendingfsm_add(TreebVacState *vstate, BlockNumber target,
							   FullTransactionId safexid);

/*
 *	_treeb_initmetapage() -- Fill a page buffer with a correct metapage image
 */
void
_treeb_initmetapage(Page page, BlockNumber rootbknum, uint32 level,
				 bool allequalimage)
{
	TreebMetaPageData *metad;
	TreebPageOpaque metaopaque;

	_treeb_pageinit(page, BLCKSZ);

	metad = TreebPageGetMeta(page);
	metad->treebm_magic = TREEB_MAGIC;
	metad->treebm_version = TREEB_VERSION;
	metad->treebm_root = rootbknum;
	metad->treebm_level = level;
	metad->treebm_fastroot = rootbknum;
	metad->treebm_fastlevel = level;
	metad->treebm_last_cleanup_num_delpages = 0;
	metad->treebm_last_cleanup_num_heap_tuples = -1.0;
	metad->treebm_allequalimage = allequalimage;

	metaopaque = TREEBPageGetOpaque(page);
	metaopaque->treebpo_flags = TREEBP_META;
	...
	((PageHeader) page)->pd_lower =
		((char *) metad + sizeof(TreebMetaPageData)) - (char *) page;
}

/*
 *	_treeb_upgrademetapage() -- Upgrade a meta-page from an old format to version
 ...
 */
void
_treeb_upgrademetapage(Page page)
{
	TreebMetaPageData *metad;
	TreebPageOpaque metaopaque PG_USED_FOR_ASSERTS_ONLY;

	metad = TreebPageGetMeta(page);
	metaopaque = TREEBPageGetOpaque(page);

	/* It must be really a meta page of upgradable version */
	Assert(metaopaque->treebpo_flags & TREEBP_META);
	Assert(metad->treebm_version < TREEB_NOVAC_VERSION);
	Assert(metad->treebm_version >= TREEB_MIN_VERSION);

	/* Set version number and fill extra fields added into version 3 */
	metad->treebm_version = TREEB_NOVAC_VERSION;
	metad->treebm_last_cleanup_num_delpages = 0;
	metad->treebm_last_cleanup_num_heap_tuples = -1.0;
	/* Only a REINDEX can set this field */
	Assert(!metad->treebm_allequalimage);
	metad->treebm_allequalimage = false;

	/* Adjust pd_lower (see _treeb_initmetapage() for details) */
	((PageHeader) page)->pd_lower =
		((char *) metad + sizeof(TreebMetaPageData)) - (char *) page;
}
 ...
 * on-the-fly upgrade using _treeb_upgrademetapage() can change the version field
 * and TREEB_NOVAC_VERSION specific fields without invalidating local cache.
 */
static TreebMetaPageData *
_treeb_getmeta(Relation rel, Buffer metabuf)
{
	Page		metapg;
	TreebPageOpaque metaopaque;
	TreebMetaPageData *metad;
	...
	metaopaque = TREEBPageGetOpaque(metapg);
	...
	if (!P_ISMETA(metaopaque) ||
		...
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
				 errmsg("index \"%s\" is not a treeb",
						RelationGetRelationName(rel))));

	if (metad->treebm_version < TREEB_MIN_VERSION ||
		metad->treebm_version > TREEB_VERSION)
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
				 errmsg("version mismatch in index \"%s\": file version %d, "
						...
						RelationGetRelationName(rel),
						metad->treebm_version, TREEB_VERSION, TREEB_MIN_VERSION)));

	return metad;
}

/*
 * _treeb_vacuum_needs_cleanup() -- Checks if index needs cleanup
 *
 * Called by treebvacuumcleanup when treebbulkdelete was never called because no
 * index tuples needed to be deleted.
 */
bool
_treeb_vacuum_needs_cleanup(Relation rel)
{
	Buffer		metabuf;
	Page		metapg;
	TreebMetaPageData *metad;
	uint32		treebm_version;
	 ...
	 */
	metabuf = _treeb_getbuf(rel, TREEB_METAPAGE, TREEB_READ);
	metapg = BufferGetPage(metabuf);
	metad = TreebPageGetMeta(metapg);
	treebm_version = metad->treebm_version;

	if (treebm_version < TREEB_NOVAC_VERSION)
	{
		/*
		 * Metapage needs to be dynamically upgraded to store fields that are
		 * only present when treebm_version >= TREEB_NOVAC_VERSION
		 */
		_treeb_relbuf(rel, metabuf);
		return true;
	}

	prev_num_delpages = metad->treebm_last_cleanup_num_delpages;
	_treeb_relbuf(rel, metabuf);
	 ...
	 * treebvacuumscan call during the ongoing treebvacuumcleanup.  For further
	 * details see the treeb/README section on placing deleted pages in the
	 * FSM.
	 */
...
/*
 * _treeb_set_cleanup_info() -- Update metapage for treebvacuumcleanup.
 *
 * Called at the end of treebvacuumcleanup, when num_delpages value has been
 * finalized.
 */
void
_treeb_set_cleanup_info(Relation rel, BlockNumber num_delpages)
{
	Buffer		metabuf;
	Page		metapg;
	TreebMetaPageData *metad;
	...
	/*
	 * On-disk compatibility note: The treebm_last_cleanup_num_delpages metapage
	 * field started out as a TransactionId field called treebm_oldest_btpo_xact.
	 ...
	 * what is now recognized as the metapage's treebm_last_cleanup_num_delpages
	 * field.  _treeb_vacuum_needs_cleanup() may even believe that this value
	 ...
	 * that there will be a call to treebvacuumscan a little earlier, which will
	 * set treebm_last_cleanup_num_delpages to a sane value when we're called.
	 *
	 * Note also that the metapage's treebm_last_cleanup_num_heap_tuples field is
	...
	/* Don't miss chance to upgrade index/metapage when TREEB_MIN_VERSION */
	if (metad->treebm_version >= TREEB_NOVAC_VERSION &&
		metad->treebm_last_cleanup_num_delpages == num_delpages)
	{
	...
	_treeb_unlockbuf(rel, metabuf);
	_treeb_lockbuf(rel, metabuf, TREEB_WRITE);
	...
	if (RelationNeedsWAL(rel))
	{
		...
		md.root = metad->treebm_root;
		md.level = metad->treebm_level;
		md.fastroot = metad->treebm_fastroot;
		md.fastlevel = metad->treebm_fastlevel;
		md.last_cleanup_num_delpages = num_delpages;
		md.allequalimage = metad->treebm_allequalimage;

		XLogRegisterBufData(0, (char *) &md, sizeof(xl_treeb_metadata));

		recptr = XLogInsert(RM_TREEB_ID, XLOG_TREEB_META_CLEANUP);
...
/*
 *	_treeb_getroot() -- Get the root page of the treeb.
 *
 *		Since the root page can move around the treeb file, we have to read
 *		its location from the metadata page, and then read the root page
 *		itself.  If no root page exists yet, we have to create one.
 *
 *		The access type parameter (TREEB_READ or TREEB_WRITE) controls whether
 *		a new root page will be created or not.  If access = TREEB_READ,
 *		and no root page exists, we just return InvalidBuffer.  For
 *		TREEB_WRITE, we try to create the root page if it doesn't exist.
 *		NOTE that the returned root page will have only a read lock set
 *		on it even if access = TREEB_WRITE!
 *
 *		If access = TREEB_WRITE, heaprel must be set; otherwise caller can just
 *		pass NULL.  See _treeb_allocbuf for an explanation.
 *
 ...
 */
Buffer
_treeb_getroot(Relation rel, Relation heaprel, int access)
{
	Buffer		metabuf;
	Buffer		rootbuf;
	Page		rootpage;
	...
	{
		metad = (TreebMetaPageData *) rel->rd_amcache;
		/* We shouldn't have cached it if any of these fail */
		Assert(metad->treebm_magic == TREEB_MAGIC);
		Assert(metad->treebm_version >= TREEB_MIN_VERSION);
		Assert(metad->treebm_version <= TREEB_VERSION);
		Assert(!metad->treebm_allequalimage ||
			   metad->treebm_version > TREEB_NOVAC_VERSION);
		Assert(metad->treebm_root != P_NONE);

		rootblkno = metad->treebm_fastroot;
		Assert(rootblkno != P_NONE);
		rootlevel = metad->treebm_fastlevel;

		rootbuf = _treeb_getbuf(rel, rootblkno, TREEB_READ);
		...
		if (!P_IGNORE(rootopaque) &&
			rootopaque->treebpo_level == rootlevel &&
	...
	metabuf = _treeb_getbuf(rel, TREEB_METAPAGE, TREEB_READ);
	metad = _treeb_getmeta(rel, metabuf);

	/* if no root page initialized yet, do it */
	if (metad->treebm_root == P_NONE)
	{
		Page		metapg;

		/* If access = TREEB_READ, caller doesn't want us to create root yet */
		...
		if (metad->treebm_root != P_NONE)
		{
			 ...
			 */
			_treeb_relbuf(rel, metabuf);
			return _treeb_getroot(rel, heaprel, access);
		}
		...
		rootopaque->treebpo_flags = (TREEBP_LEAF | TREEBP_ROOT);
		rootopaque->treebpo_level = 0;
		...
		metad->treebm_root = rootblkno;
		metad->treebm_level = 0;
		metad->treebm_fastroot = rootblkno;
		metad->treebm_fastlevel = 0;
		metad->treebm_last_cleanup_num_delpages = 0;
		metad->treebm_last_cleanup_num_heap_tuples = -1.0;
		 ...
		 */
		_treeb_unlockbuf(rel, rootbuf);
		_treeb_lockbuf(rel, rootbuf, TREEB_READ);
	...
	else
	{
		...
		rel->rd_amcache = MemoryContextAlloc(rel->rd_indexcxt,
											 sizeof(TreebMetaPageData));
		memcpy(rel->rd_amcache, metad, sizeof(TreebMetaPageData));

		/*
		 * We are done with the metapage; arrange to release it via first
		 * _treeb_relandgetbuf call
		 */
		rootbuf = metabuf;

		for (;;)
		{
			rootbuf = _treeb_relandgetbuf(rel, rootbuf, rootblkno, TREEB_READ);
			rootpage = BufferGetPage(rootbuf);
			rootopaque = TREEBPageGetOpaque(rootpage);
			...
			if (P_RIGHTMOST(rootopaque))
				elog(ERROR, "no live root page found in index \"%s\"",
					 RelationGetRelationName(rel));
			rootblkno = rootopaque->treebpo_next;
		}

		if (rootopaque->treebpo_level != rootlevel)
...
/*
 *	_treeb_gettrueroot() -- Get the true root page of the treeb.
 *
 *		This is the same as the TREEB_READ case of _treeb_getroot(), except
 ...
 */
Buffer
_treeb_gettrueroot(Relation rel)
{
	Buffer		metabuf;
	Page		metapg;
	TreebPageOpaque metaopaque;
	...
	rootblkno = metad->treebm_root;
	rootlevel = metad->treebm_level;
 ...
 *		to start any treeb index search.
 *
...
_treeb_getrootheight(Relation rel)
{
	...
	if (rel->rd_amcache == NULL)
	{
		...
		/*
		 * If there's no root page yet, _treeb_getroot() doesn't expect a cache
		 * to be made, so just stop here and report the index height is zero.
		 * (XXX perhaps _treeb_getroot() should be changed to allow this case.)
		 */
		if (metad->treebm_root == P_NONE)
		{
			_treeb_relbuf(rel, metabuf);
	...
	Assert(!metad->treebm_allequalimage ||
		   metad->treebm_version > TREEB_NOVAC_VERSION);
	Assert(metad->treebm_fastroot != P_NONE);

	return metad->treebm_fastlevel;
}

/*
 *	_treeb_metaversion() -- Get version/status info from metapage.
 *
 ...
 *		treeb.  Version 4 indexes treat heap TID as a tiebreaker attribute.
 *		pg_upgrade'd version 3 indexes need extra steps to preserve reasonable
 *		performance when inserting a new TreebScanInsert-wise duplicate tuple
 ...
 *		treebm_allequalimage will be zero'ed on heapkeyspace indexes that were
 *		pg_upgrade'd from Postgres 12.
 */
void
_treeb_metaversion(Relation rel, bool *heapkeyspace, bool *allequalimage)
{
	TreebMetaPageData *metad;
	...
	if (rel->rd_amcache == NULL)
	{
		...
		/*
		 * If there's no root page yet, _treeb_getroot() doesn't expect a cache
		 * to be made, so just stop here.  (XXX perhaps _treeb_getroot() should
		 * be changed to allow this case.)
		 */
		if (metad->treebm_root == P_NONE)
		{
			*heapkeyspace = metad->treebm_version > TREEB_NOVAC_VERSION;
			*allequalimage = metad->treebm_allequalimage;
		 ...
		 * Cache the metapage data for next time
		 *
		 * An on-the-fly version upgrade performed by _treeb_upgrademetapage()
		 * can change the treeb version for an index without invalidating any
...
/*
 *	_treeb_checkpage() -- Verify that a freshly-read page looks sane.
 */
void
_treeb_checkpage(Relation rel, Buffer buf)
{
	 ...
	 */
	if (PageGetSpecialSize(page) != MAXALIGN(sizeof(TreebPageOpaqueData)))
		ereport(ERROR,
				(errcode(ERRCODE_INDEX_CORRUPTED),
...
/*
 *	_treeb_getbuf() -- Get an existing block in a buffer, for read or write.
 *
 *		The general rule in treeb is that it's never okay to access a
 ...
 *		_treeb_checkpage to sanity-check the page, and perform Valgrind
 *		client requests that help Valgrind detect unsafe page accesses.
 *
 *		Note: raw LockBuffer() calls are disallowed in treeb; all
 *		buffer lock requests need to go through wrapper functions such
 *		as _treeb_lockbuf().
 */
Buffer
_treeb_getbuf(Relation rel, BlockNumber blkno, int access)
{
	Buffer		buf;
...
/*
 *	_treeb_allocbuf() -- Allocate a new block/page.
 *
 * Returns a write-locked buffer containing an unallocated treeb page.
 *
 ...
 */
Buffer
_treeb_allocbuf(Relation rel, Relation heaprel)
{
	Buffer		buf;
	BlockNumber blkno;
	Page		page;
	 ...
	 * it's entirely likely that _treeb_split will call us while holding a lock
	...
	for (;;)
	{
		...
		if (blkno == InvalidBlockNumber)
			break;
		buf = ReadBuffer(rel, blkno);
		if (_treeb_conditionallockbuf(rel, buf))
		{
			...
			if (PageIsNew(page))
			{
				...
				_treeb_pageinit(page, BufferGetPageSize(buf));
				return buf;
			}

			if (TreebPageIsRecyclable(page, heaprel))
			{
				/*
				...
				{
					xl_treeb_reuse_page xlrec_reuse;
					...
					xlrec_reuse.snapshotConflictHorizon = TreebPageGetDeleteXid(page);
					xlrec_reuse.isCatalogRel =
						RelationIsAccessibleInLogicalDecoding(heaprel);

					XLogBeginInsert();
					XLogRegisterData((char *) &xlrec_reuse, SizeOfTreebReusePage);

					XLogInsert(RM_TREEB_ID, XLOG_TREEB_REUSE_PAGE);
				}
	 ...
	 * risk a race condition against treebvacuumscan --- see comments therein.
	 * This forces us to repeat the valgrind request that _treeb_lockbuf()
	 * otherwise would make, as we can't use _treeb_lockbuf() without introducing
	 * a race.
	 */
...
/*
 *	_treeb_relandgetbuf() -- release a locked buffer and get another one.
 *
 * This is equivalent to _treeb_relbuf followed by _treeb_getbuf.  Also, if obuf is
 * InvalidBuffer then it reduces to just _treeb_getbuf; allowing this case
 * simplifies some callers.
 *
 * The original motivation for using this was to avoid two entries to the
 * bufmgr when one would do.  However, now it's mainly just a notational
 * convenience.  The only case where it saves work over _treeb_relbuf/_treeb_getbuf
 * is when the target page is the same one already in the buffer.
 */
Buffer
_treeb_relandgetbuf(Relation rel, Buffer obuf, BlockNumber blkno, int access)
{
	Buffer		buf;

	Assert(BlockNumberIsValid(blkno));
	if (BufferIsValid(obuf))
		_treeb_unlockbuf(rel, obuf);
...
/*
 *	_treeb_relbuf() -- release a locked buffer.
 *
 * Lock and pin (refcount) are both dropped.
 */
void
_treeb_relbuf(Relation rel, Buffer buf)
{
...
/*
 *	_treeb_lockbuf() -- lock a pinned buffer.
 *
 ...
 *
 * Note: Caller may need to call _treeb_checkpage() with buf when pin on buf
 * wasn't originally acquired in _treeb_getbuf() or _treeb_relandgetbuf().
 */
void
_treeb_lockbuf(Relation rel, Buffer buf, int access)
{
	/* LockBuffer() asserts that pin is held by this backend */
	LockBuffer(buf, access);

	/*
	 * It doesn't matter that _treeb_unlockbuf() won't get called in the event of
	 * an treeb error (e.g. a unique violation error).  That won't cause
	 * Valgrind false positives.
	 *
	 * The treeb client requests are superimposed on top of the bufmgr.c
	 * buffer pin client requests.  In the event of an treeb error the buffer
...
/*
 *	_treeb_unlockbuf() -- unlock a pinned buffer.
 */
void
_treeb_unlockbuf(Relation rel, Buffer buf)
{
	/*
...
/*
 *	_treeb_conditionallockbuf() -- conditionally TREEB_WRITE lock pinned
 *	buffer.
 *
 * Note: Caller may need to call _treeb_checkpage() with buf when pin on buf
 * wasn't originally acquired in _treeb_getbuf() or _treeb_relandgetbuf().
 */
bool
_treeb_conditionallockbuf(Relation rel, Buffer buf)
{
...
/*
 *	_treeb_upgradelockbufcleanup() -- upgrade lock to a full cleanup lock.
 */
void
_treeb_upgradelockbufcleanup(Relation rel, Buffer buf)
{
	/*
	 * Buffer is pinned and locked, which means that it is expected to be
	 * defined and addressable.  Check that proactively.
...
/*
 *	_treeb_pageinit() -- Initialize a new page.
 *
 * On return, the page header is initialized; data space is empty;
 * special space is zeroed out.
 */
void
_treeb_pageinit(Page page, Size size)
{
	PageInit(page, size, sizeof(TreebPageOpaqueData));
}

/*
 * Delete item(s) from a treeb leaf page during VACUUM.
 *
 ...
 */
void
_treeb_delitems_vacuum(Relation rel, Buffer buf,
					OffsetNumber *deletable, int ndeletable,
					TreebVacuumPosting *updatable, int nupdatable)
{
	Page		page = BufferGetPage(buf);
	...
	if (nupdatable > 0)
		updatedbuf = _treeb_delitems_update(updatable, nupdatable,
	...
	/*
	 * Clear the TREEBP_HAS_GARBAGE page flag.
	 *
	...
	if (needswal)
	{
		...
		XLogRegisterData((char *) &xlrec_vacuum, SizeOfTreebVacuum);
		...
		recptr = XLogInsert(RM_TREEB_ID, XLOG_TREEB_VACUUM);
	...
	if (updatedbuf != NULL)
		pfree(updatedbuf);
	/* free tuples allocated within _treeb_delitems_update() */
	for (int i = 0; i < nupdatable; i++)
		pfree(updatable[i]->itup);
}

/*
 * Delete item(s) from a treeb leaf page during single-page cleanup.
 *
 ...
 * (a version that lacks the TIDs that are to be deleted).
 *
 * This is nearly the same as _treeb_delitems_vacuum as far as what it does to
 ...
 * conflicts.  The other difference is that only _treeb_delitems_vacuum will
 * clear page's VACUUM cycle ID.
 */
static void
_treeb_delitems_delete(Relation rel, Buffer buf,
					...
					TreebVacuumPosting *updatable, int nupdatable)
{
	...
	/* Handle updates and deletes just like _treeb_delitems_vacuum */
	for (int i = 0; i < nupdatable; i++)
	{
		OffsetNumber updatedoffset = updatedoffsets[i];
		IndexTuple	itup;
	...
	/*
	 * Unlike _treeb_delitems_vacuum, we *must not* clear the vacuum cycle ID at
	...
	if (needswal)
	{
		...
		XLogBeginInsert();
		XLogRegisterBuffer(0, buf, REGBUF_STANDARD);
		XLogRegisterData((char *) &xlrec_delete, SizeOfTreebDelete);
		...
		recptr = XLogInsert(RM_TREEB_ID, XLOG_TREEB_DELETE);
 ...
 * the tuple.  Performs steps common to both _treeb_delitems_vacuum and
 * _treeb_delitems_delete.  These steps must take place before each function's
 * critical section begins.
 *
 * updatable and nupdatable are inputs, though note that we will use
 * _treeb_update_posting() to replace the original itup with a pointer to a final
 ...
 *
 * Returns buffer consisting of an array of xl_treeb_update structs that
 ...
 */
static char *
_treeb_delitems_update(TreebVacuumPosting *updatable, int nupdatable,
					...
					bool needswal)
{
	...
	for (int i = 0; i < nupdatable; i++)
	{
		TreebVacuumPosting vacposting = updatable[i];
		Size		itemsz;

		/* Replace work area IndexTuple with updated version */
		_treeb_update_posting(vacposting);

		/* Keep track of size of xl_treeb_update for updatedbuf in passing */
		itemsz = SizeOfTreebUpdate + vacposting->ndeletedtids * sizeof(uint16);
		buflen += itemsz;
...
/*
 * Comparator used by _treeb_delitems_delete_check() to restore deltids array
 * back to its original leaf-page-wise sort order
 */
static int
_treeb_delitems_cmp(const void *a, const void *b)
{
...
/*
 * Try to delete item(s) from a treeb leaf page during single-page cleanup.
 *
 * treeb interface to table_index_delete_tuples().  Deletes a subset of index
 ...
 */
void
_treeb_delitems_delete_check(Relation rel, Buffer buf, Relation heapRel,
						  TM_IndexDeleteOp *delstate)
{
	...
	/*
	 * Construct a leaf-page-wise description of what _treeb_delitems_delete()
	...
	for (int i = 0; i < delstate->ndeltids; i++)
	{
		...
		if (idxoffnum == postingidxoffnum)
		{
			...
			Assert(ItemPointerCompare(TreebTupleGetHeapTID(itup),
									  &delstate->deltids[i].tid) < 0);
			Assert(ItemPointerCompare(TreebTupleGetMaxHeapTID(itup),
									  &delstate->deltids[i].tid) >= 0);
			continue;
		}
		...
		nitem = TreebTupleGetNPosting(itup);
		for (int p = 0; p < nitem; p++)
		{
			ItemPointer ptid = TreebTupleGetPostingN(itup, p);
			int			ptidcmp = -1;
	...
	_treeb_delitems_delete(rel, buf, snapshotConflictHorizon, isCatalogRel,
...
/*
 * Check that leftsib page (the treebpo_prev of target page) is not marked with
 ...
 */
static bool
_treeb_leftsib_splitflag(Relation rel, BlockNumber leftsib, BlockNumber target)
{
	Buffer		buf;
	Page		page;
	TreebPageOpaque opaque;
	...
	buf = _treeb_getbuf(rel, leftsib, TREEB_READ);
	page = BufferGetPage(buf);
	opaque = TREEBPageGetOpaque(page);
	 ...
	 */
	result = (opaque->treebpo_next == target && P_INCOMPLETE_SPLIT(opaque));
	_treeb_relbuf(rel, buf);
...
/*
 * Check that leafrightsib page (the treebpo_next of target leaf page) is not
 ...
 * again).
 *
 * _treeb_leftsib_splitflag() is called for both leaf pages and internal pages.
 * _treeb_rightsib_halfdeadflag() is only called for leaf pages, though.  This is
 ...
 */
static bool
_treeb_rightsib_halfdeadflag(Relation rel, BlockNumber leafrightsib)
{
	Buffer		buf;
	Page		page;
	TreebPageOpaque opaque;
	...
	buf = _treeb_getbuf(rel, leafrightsib, TREEB_READ);
	page = BufferGetPage(buf);
	opaque = TREEBPageGetOpaque(page);
...
/*
 * _treeb_pagedel() -- Delete a leaf page from the b-tree, if legal to do so.
 *
 * This action unlinks the leaf page from the b-tree structure, removing all
 * pointers leading to it --- but not touching its own left and right links.
 * The page cannot be physically reclaimed right away, since other processes
 * may currently be trying to follow links leading to the page; they have to
 * be allowed to use its right-link to recover.  See treeb/README.
 *
 ...
 */
void
_treeb_pagedel(Relation rel, Buffer leafbuf, TreebVacState *vstate)
{
	...
	for (;;)
	{
		 ...
		 * _treeb_leftsib_splitflag().  On the first iteration, we temporarily
		...
		if (!P_ISHALFDEAD(opaque))
		{
			...
			if (!stack)
			{
				...
				if (_treeb_leftsib_splitflag(rel, leftsib, leafblkno))
				{
				 ...
				 *
				 * _treeb_search searches for the leaf page that contains any
				 * matching non-pivot tuples, but we need it to "search" for
				 * the high key pivot from the page that we're set to delete.
				 * Compensate for the mismatch by having _treeb_search locate the
				 * last position < equal-to-untruncated-prefix non-pivots.
				 */
				itup_key = _treeb_mkscankey(rel, targetkey);

				/* Set up a TreebLessStrategyNumber-like insertion scan key */
				itup_key->nextkey = false;
				itup_key->backward = true;
				stack = _treeb_search(rel, NULL, itup_key, &sleafbuf, TREEB_READ);
				/* won't need a second lock or pin on leafbuf */
				_treeb_relbuf(rel, sleafbuf);

				/*
				 * Re-lock the leaf page, and start over to use our stack
				 * within _treeb_mark_page_halfdead.  We must do it that way
				 ...
				 * original lock was dropped, but before _treeb_search finished
				 ...
				 */
				_treeb_lockbuf(rel, leafbuf, TREEB_WRITE);
				continue;
			}
			 ...
			 * deleted.  If it's safe then _treeb_mark_page_halfdead will also
			 * perform the first phase of deletion, which includes marking the
			 * leafbuf page half-dead.
			 */
			Assert(P_ISLEAF(opaque) && !P_IGNORE(opaque));
			if (!_treeb_mark_page_halfdead(rel, vstate->info->heaprel, leafbuf,
										stack))
			{
				_treeb_relbuf(rel, leafbuf);
				return;
		 ...
		 * _treeb_unlink_halfdead_page unlinks the topmost page from the subtree,
		...
		{
			/* Check for interrupts in _treeb_unlink_halfdead_page */
			if (!_treeb_unlink_halfdead_page(rel, leafbuf, scanblkno,
										  &rightsib_empty, vstate))
			{
				/*
				 * _treeb_unlink_halfdead_page should never fail, since we
				 * established that deletion is generally safe in
				 * _treeb_mark_page_halfdead -- index must be corrupt.
				 *
				 * Note that _treeb_unlink_halfdead_page already released the
		 ...
		 *
		 * Note: This relies on the assumption that _treeb_getstackbuf() will be
		 ...
		 * original leaf page reached by _treeb_search()). It will even update
		 * the descent stack each time we loop around, avoiding repeated work.
		 */
		if (!rightsib_empty)
			break;

		leafbuf = _treeb_getbuf(rel, rightsib, TREEB_WRITE);
	}
}
 ...
 * call to _treeb_lock_subtree_parent will need to allocate a new index page to
 * complete a page split.  Every call to _treeb_allocbuf needs to pass a heaprel.
 *
 ...
 */
static bool
_treeb_mark_page_halfdead(Relation rel, Relation heaprel, Buffer leafbuf,
					   TreebStack stack)
{
	 ...
	 */
	leafblkno = BufferGetBlockNumber(leafbuf);
	leafrightsib = opaque->treebpo_next;
	 ...
	 */
	if (_treeb_rightsib_halfdeadflag(rel, leafrightsib))
	{
	 ...
	 * "subtree parent").  _treeb_lock_subtree_parent() locks the subtree parent
	...
	if (!_treeb_lock_subtree_parent(rel, heaprel, leafblkno, stack,
	...
	/*
	 * This is just an assertion because _treeb_lock_subtree_parent should have
	 * guaranteed tuple has the expected contents
	 */
	itemid = PageGetItemId(page, poffset);
	itup = (IndexTuple) PageGetItem(page, itemid);
	Assert(TreebTupleGetDownLink(itup) == topparent);
#endif
	 ...
	 * where _treeb_lock_subtree_parent() cannot "re-find" leafbuf's downlink.)
	 */
	if (TreebTupleGetDownLink(itup) != topparentrightsib)
	{
		ereport(LOG,
				(errcode(ERRCODE_INDEX_CORRUPTED),
				 errmsg_internal("right sibling %u of block %u is not next child %u of block %u in index \"%s\"",
								 topparentrightsib, topparent,
								 TreebTupleGetDownLink(itup),
								 BufferGetBlockNumber(subtreeparent),
								 RelationGetRelationName(rel))));

		_treeb_relbuf(rel, subtreeparent);
		Assert(false);
		return false;
	}
	...
	itemid = PageGetItemId(page, poffset);
	itup = (IndexTuple) PageGetItem(page, itemid);
	TreebTupleSetDownLink(itup, topparentrightsib);
	...
	if (topparent != leafblkno)
		TreebTupleSetTopParent(&trunctuple, topparent);
	else
		TreebTupleSetTopParent(&trunctuple, InvalidBlockNumber);
	...
	if (RelationNeedsWAL(rel))
	{
		xl_treeb_mark_page_halfdead xlrec;
		XLogRecPtr	recptr;
		...
		page = BufferGetPage(leafbuf);
		opaque = TREEBPageGetOpaque(page);
		xlrec.leftblk = opaque->treebpo_prev;
		xlrec.rightblk = opaque->treebpo_next;

		XLogRegisterData((char *) &xlrec, SizeOfTreebMarkPageHalfDead);

		recptr = XLogInsert(RM_TREEB_ID, XLOG_TREEB_MARK_PAGE_HALFDEAD);
 ...
 */
static bool
_treeb_unlink_halfdead_page(Relation rel, Buffer leafbuf, BlockNumber scanblkno,
						 bool *rightsib_empty, TreebVacState *vstate)
{
	...
	target = TreebTupleGetTopParent(leafhikey);
	leafleftsib = opaque->treebpo_prev;
	leafrightsib = opaque->treebpo_next;
	...
	else
	{
		...
		buf = _treeb_getbuf(rel, target, TREEB_READ);
		page = BufferGetPage(buf);
		opaque = TREEBPageGetOpaque(page);
		leftsib = opaque->treebpo_prev;
		targetlevel = opaque->treebpo_level;
	...
	if (leftsib != P_NONE)
	{
		lbuf = _treeb_getbuf(rel, leftsib, TREEB_WRITE);
		page = BufferGetPage(lbuf);
		opaque = TREEBPageGetOpaque(page);
		while (P_ISDELETED(opaque) || opaque->treebpo_next != target)
		{
			...
			if (P_RIGHTMOST(opaque) || P_ISDELETED(opaque) ||
				leftsib == opaque->treebpo_next)
				leftsibvalid = false;

			leftsib = opaque->treebpo_next;
			_treeb_relbuf(rel, lbuf);
	...
	else
	{
		...
		leaftopparent = TreebTupleGetDownLink(finaldataitem);
	 ...
	 */
	rightsib = opaque->treebpo_next;
	rbuf = _treeb_getbuf(rel, rightsib, TREEB_WRITE);
	...
	*rightsib_empty = (P_FIRSTDATAKEY(opaque) > PageGetMaxOffsetNumber(page));
	...
	if (leftsib == P_NONE && rightsib_is_rightmost)
	{
		...
		if (P_RIGHTMOST(opaque))
		{
			...
			/*
			 * The expected case here is treebm_fastlevel == targetlevel+1; if
			 * the fastlevel is <= targetlevel, something is wrong, and we
			 * choose to overwrite it to fix it.
			 */
			if (metad->treebm_fastlevel > targetlevel + 1)
			{
				/* no update wanted */
				_treeb_relbuf(rel, metabuf);
	...
	if (BufferIsValid(lbuf))
	{
		page = BufferGetPage(lbuf);
		opaque = TREEBPageGetOpaque(page);
		Assert(opaque->treebpo_next == target);
		opaque->treebpo_next = rightsib;
	}
	page = BufferGetPage(rbuf);
	opaque = TREEBPageGetOpaque(page);
	Assert(opaque->treebpo_prev == target);
	opaque->treebpo_prev = leftsib;
	 ...
	 */
	if (target != leafblkno)
		TreebTupleSetTopParent(leafhikey, leaftopparent);
	...
	TreebPageSetDeleted(page, safexid);
	opaque->treebpo_cycleid = 0;
	...
	if (BufferIsValid(metabuf))
	{
		...
		if (metad->treebm_version < TREEB_NOVAC_VERSION)
			_treeb_upgrademetapage(metapg);
		metad->treebm_fastroot = rightsib;
		metad->treebm_fastlevel = targetlevel;
		MarkBufferDirty(metabuf);
	}
	...
	if (RelationNeedsWAL(rel))
	{
		...
		XLogRegisterData((char *) &xlrec, SizeOfTreebUnlinkPage);
		...
		if (BufferIsValid(metabuf))
		{
			...
			XLogRegisterBufData(4, (char *) &xlmeta, sizeof(xl_treeb_metadata));
			xlinfo = XLOG_TREEB_UNLINK_PAGE_META;
		}
		else
			xlinfo = XLOG_TREEB_UNLINK_PAGE;
	 ...
	 * treebvacuumpage() will count deleted pages that have yet to become
	 ...
	 * candidate to place in the FSM at the end of the current treebvacuumscan()
	 * call.
	 */
	_treeb_pendingfsm_add(vstate, target, safexid);
 ...
 * parent block in the leafbuf page using TreebTupleSetTopParent()).
 */
static bool
_treeb_lock_subtree_parent(Relation rel, Relation heaprel, BlockNumber child,
						TreebStack stack, Buffer *subtreeparent,
						...
						BlockNumber *topparentrightsib)
{
	 ...
	 */
	pbuf = _treeb_getstackbuf(rel, heaprel, stack, child);
	if (pbuf == InvalidBuffer)
	{
		 ...
		 *
		 * Note: _treeb_getstackbuf() recovers from concurrent page splits that
	...
	parent = stack->treebs_blkno;
	parentoffset = stack->treebs_offset;

	page = BufferGetPage(pbuf);
	opaque = TREEBPageGetOpaque(page);
	maxoff = PageGetMaxOffsetNumber(page);
	leftsibparent = opaque->treebpo_prev;

	/*
	 * _treeb_getstackbuf() completes page splits on returned parent buffer when
	 * required.
	 *
	 ...
	 */
	*topparent = parent;
	*topparentrightsib = opaque->treebpo_next;
	 ...
	 */
	if (_treeb_leftsib_splitflag(rel, leftsibparent, parent))
		return false;

	/* Recurse to examine child page's grandparent page */
	return _treeb_lock_subtree_parent(rel, heaprel, parent, stack->treebs_parent,
								   subtreeparent, poffset,
								   topparent, topparentrightsib);
}

/*
 * Initialize local memory state used by VACUUM for _treeb_pendingfsm_finalize
 * optimization.
 *
 * Called at the start of a treebvacuumscan().  Caller's cleanuponly argument
 * indicates if ongoing VACUUM has not (and will not) call treebbulkdelete().
 *
 ...
 */
void
_treeb_pendingfsm_init(Relation rel, TreebVacState *vstate, bool cleanuponly)
{
	int64		maxbufsize;

	/*
	 * Don't bother with optimization in cleanup-only case -- we don't expect
	 * any newly deleted pages.  Besides, cleanup-only calls to treebvacuumscan()
	 ...
	 */
	vstate->bufsize = 256;
	maxbufsize = (work_mem * 1024L) / sizeof(TreebPendingFSM);
	maxbufsize = Min(maxbufsize, INT_MAX);
	maxbufsize = Min(maxbufsize, MaxAllocSize / sizeof(TreebPendingFSM));
	...
	vstate->pendingpages = palloc(sizeof(TreebPendingFSM) * vstate->bufsize);
	vstate->npendingpages = 0;
}

/*
 * Place any newly deleted pages (i.e. pages that _treeb_pagedel() deleted during
 * the ongoing VACUUM operation) into the free space map -- though only when
 * it is actually safe to do so by now.
 *
 * Called at the end of a treebvacuumscan(), just before free space map vacuuming
 * takes place.
 *
 * Frees memory allocated by _treeb_pendingfsm_init(), if any.
 */
void
_treeb_pendingfsm_finalize(Relation rel, TreebVacState *vstate)
{
	IndexBulkDeleteResult *stats = vstate->stats;
	...
	for (int i = 0; i < vstate->npendingpages; i++)
	{
		...
		/*
		 * Do the equivalent of checking TreebPageIsRecyclable(), but without
		 * accessing the page again a second time.
		 *
		 * Give up on finding the first non-recyclable page -- all later pages
		 * must be non-recyclable too, since _treeb_pendingfsm_add() adds pages
...
/*
 * Maintain array of pages that were deleted during current treebvacuumscan()
 * call, for use in _treeb_pendingfsm_finalize()
 */
static void
_treeb_pendingfsm_add(TreebVacState *vstate,
				   BlockNumber target,
				   FullTransactionId safexid)
{
	...
	/*
	 * Verify an assumption made by _treeb_pendingfsm_finalize(): pages from the
	 ...
	 * optimization in _treeb_pendingfsm_init().
	 */
	...
	if (vstate->npendingpages == vstate->bufsize)
	{
		...
		vstate->pendingpages =
			repalloc(vstate->pendingpages,
					 sizeof(TreebPendingFSM) * vstate->bufsize);
	}
