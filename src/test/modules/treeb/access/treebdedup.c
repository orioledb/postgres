 ...
 *
 * treebdedup.c
 *	  Deduplicate or bottom-up delete items in Postgres treebs.
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/treeb/treebdedup.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static void _treeb_bottomupdel_finish_pending(Page page, TreebDedupState state,
										   TM_IndexDeleteOp *delstate);
static bool _treeb_do_singleval(Relation rel, Page page, TreebDedupState state,
							 OffsetNumber minoff, IndexTuple newitem);
static void _treeb_singleval_fillfactor(Page page, TreebDedupState state,
									 Size newitemsz);
#ifdef USE_ASSERT_CHECKING
static bool _treeb_posting_valid(IndexTuple posting);
#endif
 ...
 * the page for an anticipated page split that uses treebsplitloc.c's own single
 * value strategy.  Our high level goal is to delay merging the untouched
 * tuples until after the page splits.
 *
 * When a call to _treeb_bottomupdel_pass() just took place (and failed), our
 ...
 */
void
_treeb_dedup_pass(Relation rel, Buffer buf, IndexTuple newitem, Size newitemsz,
			   bool bottomupdedup)
{
	OffsetNumber offnum,
				minoff,
	...
	state->nmaxitems = 0;
	state->maxpostingsize = Min(TreebMaxItemSize(page) / 2, INDEX_SIZE_MASK);
	 ...
	 */
	if (!bottomupdedup)
		singlevalstrat = _treeb_do_singleval(rel, page, state, minoff, newitem);
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		if (offnum == minoff)
		{
			...
			_treeb_dedup_start_pending(state, itup, offnum);
				 ...
				 _treeb_keep_natts_fast(rel, state->base, itup) > nkeyatts &&
				 _treeb_dedup_save_htid(state, itup))
		{
			/*
		...
		else
		{
			 ...
			 */
			pagesaving += _treeb_dedup_finish_pending(newpage, state);
			...
			if (singlevalstrat)
			{
				...
				if (state->nmaxitems == 5)
					_treeb_singleval_fillfactor(page, state, newitemsz);
	 ...
	 * don't do that because there is some small value in treebsplitloc.c always
	 ...
	 *
	 * Clear the TREEBP_HAS_GARBAGE page flag.  The index must be a heapkeyspace
	 * index, and as such we'll never pay attention to TREEBP_HAS_GARBAGE anyway.
	...
	{
		XLogRecPtr	recptr;
		xl_treeb_dedup xlrec_dedup;

		xlrec_dedup.nintervals = state->nintervals;

		XLogBeginInsert();
		XLogRegisterBuffer(0, buf, REGBUF_STANDARD);
		XLogRegisterData((char *) &xlrec_dedup, SizeOfTreebDedup);
		...
		XLogRegisterBufData(0, (char *) state->intervals,
							state->nintervals * sizeof(TreebDedupInterval));

		recptr = XLogInsert(RM_TREEB_ID, XLOG_TREEB_DEDUP);
 ...
 * tuples together.
 *
 * See treeb/README for details on the design of treeb bottom-up deletion.
 ...
 */
bool
_treeb_bottomupdel_pass(Relation rel, Buffer buf, Relation heapRel,
					 Size newitemsz)
{
	OffsetNumber offnum,
				minoff,
		 ...
		 offnum = OffsetNumberNext(offnum))
	{
		...
		else if (_treeb_keep_natts_fast(rel, state->base, itup) > nkeyatts &&
				 _treeb_dedup_save_htid(state, itup))
		{
		...
		else
		{
			...
			_treeb_bottomupdel_finish_pending(page, state, &delstate);
	...
	_treeb_delitems_delete_check(rel, buf, heapRel, &delstate);
						...
						OffsetNumber baseoff)
{
	...
	else
	{
		...
		nposting = TreebTupleGetNPosting(base);
		memcpy(state->htids, TreebTupleGetPosting(base),
			   sizeof(ItemPointerData) * nposting);
		state->nhtids = nposting;
		/* basetupsize should not include existing posting list */
		state->basetupsize = TreebTupleGetPostingOffset(base);
	}
 ...
 */
bool
_treeb_dedup_save_htid(TreebDedupState state, IndexTuple itup)
{
	int			nhtids;
	ItemPointer htids;
	Size		mergedtupsz;
	...
	{
		nhtids = TreebTupleGetNPosting(itup);
		htids = TreebTupleGetPosting(itup);
	}
	 ...
	 *
	 * This calculation needs to match the code used within _treeb_form_posting()
	...
	if (mergedtupsz > state->maxpostingsize)
	{
		 ...
		 * deduplication pass, or current pass).  See _treeb_do_singleval()
		 * comments.
		 */
		if (state->nhtids > 50)
			state->nmaxitems++;
 ...
 */
Size
_treeb_dedup_finish_pending(Page newpage, TreebDedupState state)
{
	OffsetNumber tupoff;
	Size		tuplesz;
	Size		spacesaving;
	...
	if (state->nitems == 1)
	{
		...
		Assert(tuplesz <= TreebMaxItemSize(newpage));
	...
	else
	{
		...
		final = _treeb_form_posting(state->base, state->htids, state->nhtids);
 ...
 */
static void
_treeb_bottomupdel_finish_pending(Page page, TreebDedupState state,
							   TM_IndexDeleteOp *delstate)
{
	...
	for (int i = 0; i < state->nitems; i++)
	{
		...
		else
		{
			...
			Assert(_treeb_posting_valid(itup));
			...
			if (dupinterval)
			{
				...
				mintid = TreebTupleGetHeapTID(itup);
				midtid = TreebTupleGetPostingN(itup, nitem / 2);
				maxtid = TreebTupleGetMaxHeapTID(itup);
			...
			{
				ItemPointer htid = TreebTupleGetPostingN(itup, p);
 ...
 * treebsplitloc.c (which uses its own single value strategy) will find a useful
 ...
 * the page finally splits, it should end up TREEB_SINGLEVAL_FILLFACTOR% full,
 ...
 * reaches _treeb_singleval_fillfactor(), and so will deliberately leave behind a
 ...
 */
static bool
_treeb_do_singleval(Relation rel, Page page, TreebDedupState state,
				 OffsetNumber minoff, IndexTuple newitem)
{
	int			nkeyatts = IndexRelationGetNumberOfKeyAttributes(rel);
	ItemId		itemid;
	...
	if (_treeb_keep_natts_fast(rel, newitem, itup) > nkeyatts)
	{
 ...
 * plain non-pivot tuples -- see _treeb_do_singleval() comments.)
 */
static void
_treeb_singleval_fillfactor(Page page, TreebDedupState state, Size newitemsz)
{
	Size		leftfree;
	int			reduction;

	/* This calculation needs to match treebsplitloc.c */
	leftfree = PageGetPageSize(page) - SizeOfPageHeaderData -
		MAXALIGN(sizeof(TreebPageOpaqueData));
	 ...
	 * half of page
	 */
	reduction = leftfree * ((100 - TREEB_SINGLEVAL_FILLFACTOR) / 100.0);
 ...
 */
IndexTuple
_treeb_form_posting(IndexTuple base, ItemPointer htids, int nhtids)
{
	uint32		keysize,
				newsize;
	IndexTuple	itup;
	...
	if (TreebTupleIsPosting(base))
		keysize = TreebTupleGetPostingOffset(base);
	else
	...
	if (nhtids > 1)
	{
		...
		TreebTupleSetPosting(itup, nhtids, keysize);
		memcpy(TreebTupleGetPosting(itup), htids,
			   sizeof(ItemPointerData) * nhtids);
		Assert(_treeb_posting_valid(itup));
	}
	else
 ...
 */
void
_treeb_update_posting(TreebVacuumPosting vacposting)
{
	...
	nhtids = TreebTupleGetNPosting(origtuple) - vacposting->ndeletedtids;

	Assert(_treeb_posting_valid(origtuple));
	Assert(nhtids > 0 && nhtids < TreebTupleGetNPosting(origtuple));

	/*
	 * Determine final size of new tuple.
	 *
	 * This calculation needs to match the code used within _treeb_form_posting()
	 * for new posting list tuples.  We avoid calling _treeb_form_posting() here
	 * to save ourselves a second memory allocation for a htids workspace.
	 */
	keysize = TreebTupleGetPostingOffset(origtuple);
	if (nhtids > 1)
		newsize = MAXALIGN(keysize +
						   nhtids * sizeof(ItemPointerData));
	else
	...
	ui = 0;
	d = 0;
	for (int i = 0; i < TreebTupleGetNPosting(origtuple); i++)
	{
		...
		htids[ui++] = *TreebTupleGetPostingN(origtuple, i);
	}
	Assert(ui == nhtids);
	Assert(d == vacposting->ndeletedtids);
	Assert(nhtids == 1 || _treeb_posting_valid(itup));
 ...
 * in order to support treeb features like page-level prefix compression.
 *
 * See treeb/README for details on the design of posting list splits.
 */
IndexTuple
_treeb_swap_posting(IndexTuple newitem, IndexTuple oposting, int postingoff)
{
	int			nhtids;
	char	   *replacepos;
	...
	nhtids = TreebTupleGetNPosting(oposting);
	Assert(_treeb_posting_valid(oposting));

	/*
	 * The postingoff argument originated as a _treeb_binsrch_posting() return
	...
	replacepos = (char *) TreebTupleGetPostingN(nposting, postingoff);
	replaceposright = (char *) TreebTupleGetPostingN(nposting, postingoff + 1);
	...
	Assert(!TreebTupleIsPivot(newitem) && !TreebTupleIsPosting(newitem));
	ItemPointerCopy(&newitem->t_tid, (ItemPointer) replacepos);

	/* Now copy oposting's rightmost/max TID into new item (final new TID) */
	ItemPointerCopy(TreebTupleGetMaxHeapTID(oposting), &newitem->t_tid);

	Assert(ItemPointerCompare(TreebTupleGetMaxHeapTID(nposting),
							  TreebTupleGetHeapTID(newitem)) < 0);
	Assert(_treeb_posting_valid(nposting));
...
#ifdef USE_ASSERT_CHECKING
static bool
_treeb_posting_valid(IndexTuple posting)
{
	ItemPointerData last;
	ItemPointer htid;

	if (!TreebTupleIsPosting(posting) || TreebTupleGetNPosting(posting) < 2)
		return false;

	/* Remember first heap TID for loop */
	ItemPointerCopy(TreebTupleGetHeapTID(posting), &last);
	if (!ItemPointerIsValid(&last))
		return false;

	/* Iterate, starting from second TID */
	for (int i = 1; i < TreebTupleGetNPosting(posting); i++)
	{
		htid = TreebTupleGetPostingN(posting, i);
