 ...
 *
 * treebutils.c
 *	  Utility code for Postgres treeb implementation.
 *
 ...
 *
 * IDENTIFICATION
 *	  src/backend/access/treeb/treebutils.c
 *
 *-------------------------------------------------------------------------
 */
...
typedef struct TreebSortArrayContext
{
	FmgrInfo   *sortproc;
	Oid			collation;
	bool		reverse;
} TreebSortArrayContext;

typedef struct TreebScanKeyPreproc
{
	ScanKey		skey;
	int			ikey;
	int			arrayidx;
...
static void _treeb_setup_array_cmp(IndexScanDesc scan, ScanKey skey, Oid elemtype,
								FmgrInfo *orderproc, FmgrInfo **sortprocp);
static Datum _treeb_find_extreme_element(IndexScanDesc scan, ScanKey skey,
									  Oid elemtype, StrategyNumber strat,
									  Datum *elems, int nelems);
static int	_treeb_sort_array_elements(ScanKey skey, FmgrInfo *sortproc,
									bool reverse, Datum *elems, int nelems);
static bool _treeb_merge_arrays(IndexScanDesc scan, ScanKey skey,
...
static bool _treeb_compare_array_scankey_args(IndexScanDesc scan,
										   ScanKey arraysk, ScanKey skey,
										   FmgrInfo *orderproc, TreebArrayKeyInfo *array,
										   bool *qual_ok);
static ScanKey _treeb_preprocess_array_keys(IndexScanDesc scan);
static void _treeb_preprocess_array_keys_final(IndexScanDesc scan, int *keyDataMap);
static int	_treeb_compare_array_elements(const void *a, const void *b, void *arg);
static inline int32 _treeb_compare_array_skey(FmgrInfo *orderproc,
										   Datum tupdatum, bool tupnull,
										   Datum arrdatum, ScanKey cur);
static int	_treeb_binsrch_array_skey(FmgrInfo *orderproc,
								   bool cur_elem_trig, ScanDirection dir,
								   Datum tupdatum, bool tupnull,
								   TreebArrayKeyInfo *array, ScanKey cur,
								   int32 *set_elem_result);
static bool _treeb_advance_array_keys_increment(IndexScanDesc scan, ScanDirection dir);
static void _treeb_rewind_nonrequired_arrays(IndexScanDesc scan, ScanDirection dir);
static bool _treeb_tuple_before_array_skeys(IndexScanDesc scan, ScanDirection dir,
										 IndexTuple tuple, TupleDesc tupdesc, int tupnatts,
										 bool readpagetup, int sktrig, bool *scanBehind);
static bool _treeb_advance_array_keys(IndexScanDesc scan, TREEBReadPageState *pstate,
								   IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
								   int sktrig, bool sktrig_required);
#ifdef USE_ASSERT_CHECKING
static bool _bt_verify_arrays_treeb_first(IndexScanDesc scan, ScanDirection dir);
static bool _treeb_verify_keys_with_arraykeys(IndexScanDesc scan);
#endif
static bool _treeb_compare_scankey_args(IndexScanDesc scan, ScanKey op,
									 ScanKey leftarg, ScanKey rightarg,
									 TreebArrayKeyInfo *array, FmgrInfo *orderproc,
									 bool *result);
static bool _treeb_fix_scankey_strategy(ScanKey skey, int16 *indoption);
static void _treeb_mark_scankey_required(ScanKey skey);
static bool _treeb_check_compare(IndexScanDesc scan, ScanDirection dir,
							  IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
							  bool advancenonrequired, bool prechecked, bool firstmatch,
							  bool *continuescan, int *ikey);
static bool _treeb_check_rowcompare(ScanKey skey,
								 IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
								 ScanDirection dir, bool *continuescan);
static void _treeb_checkkeys_look_ahead(IndexScanDesc scan, TREEBReadPageState *pstate,
									 int tupnatts, TupleDesc tupdesc);
static int	_treeb_keep_natts(Relation rel, IndexTuple lastleft,
 ...
 *
 *		The result is intended for use with _treeb_compare() and _treeb_truncate().
 ...
 *		for _treeb_truncate()) can pass a NULL index tuple.  The scankey will
 ...
 *		is passed; CREATE INDEX callers call _treeb_allequalimage() to set the
 *		field themselves.
 */
TreebScanInsert
_treeb_mkscankey(Relation rel, IndexTuple itup)
{
	TreebScanInsert key;
	ScanKey		skey;
	TupleDesc	itupdesc;
	...
	tupnatts = itup ? TreebTupleGetNAtts(itup, rel) : 0;
	 ...
	 * scan key.
	 */
	key = palloc(offsetof(TreebScanInsertData, scankeys) +
				 sizeof(ScanKeyData) * indnkeyatts);
	if (itup)
		_treeb_metaversion(rel, &key->heapkeyspace, &key->allequalimage);
	else
	{
	...
	key->nextkey = false;		/* usual case, required by treebinsert */
	key->backward = false;		/* usual case, required by treebinsert */
	key->keysz = Min(indnkeyatts, tupnatts);
	key->scantid = key->heapkeyspace && itup ?
		TreebTupleGetHeapTID(itup) : NULL;
	skey = key->scankeys;
	for (i = 0; i < indnkeyatts; i++)
	{
		 ...
		 */
		procinfo = index_getprocinfo(rel, i + 1, TREEBORDER_PROC);
		...
		flags = (null ? SK_ISNULL : 0) | (indoption[i] << SK_TREEB_INDOPTION_SHIFT);
...
/*
 * free a retracement stack made by _treeb_search.
 */
void
_treeb_freestack(TreebStack stack)
{
	TreebStack		ostack;
...
/*
 *	_treeb_preprocess_array_keys() -- Preprocess SK_SEARCHARRAY scan keys
 *
 * If there are any SK_SEARCHARRAY scan keys, deconstruct the array(s) and
 * set up TreebArrayKeyInfo info for each one that is an equality-type key.
 ...
 *
 * It is convenient for _treeb_preprocess_keys caller to have to deal with no
 ...
 *
 * We set the scan key references from the scan's TreebArrayKeyInfo info array to
 * offsets into the temp modified input array returned to caller.  Scans that
 * have array keys should call _treeb_preprocess_array_keys_final when standard
 ...
 * scribbling on scan->keyData, is that callers are permitted to call treebrescan
 * without supplying a new set of scankey data.
 */
static ScanKey
_treeb_preprocess_array_keys(IndexScanDesc scan)
{
	...
	so->arrayKeys = (TreebArrayKeyInfo *) palloc(numArrayKeys * sizeof(TreebArrayKeyInfo));

	/* Allocate space for ORDER procs used to help _treeb_checkkeys */
	...
	for (int i = 0; i < numberOfKeys; i++)
	{
		 ...
		 * all treeb operators are strict.
		 */
		num_nonnulls = 0;
		...
		switch (cur->sk_strategy)
		{
			...
			case TreebLessEqualStrategyNumber:
				cur->sk_argument =
					_treeb_find_extreme_element(scan, cur, elemtype,
		 ...
		 */
		_treeb_setup_array_cmp(scan, cur, elemtype,
		...
		num_elems = _treeb_sort_array_elements(cur, sortprocp, reverse,
											elem_values, num_nonnulls);

		if (origarrayatt == cur->sk_attno)
		{
			TreebArrayKeyInfo *orig = &so->arrayKeys[origarraykey];
			...
			Assert(arrayKeyData[orig->scan_key].sk_collation ==
				   cur->sk_collation);
			if (_treeb_merge_arrays(scan, cur, sortprocp, reverse,
								 ...
								 elem_values, num_elems))
			{
				...
				/*
				 * Indicate to _treeb_preprocess_keys caller that it must ignore
				 * this scan key
				 */
		...
		/*
		 * And set up the TreebArrayKeyInfo data.
		 *
		 * Note: _treeb_preprocess_array_keys_final will fix-up each array's
...
/*
 *	_treeb_preprocess_array_keys_final() -- fix up array scan key references
 *
 * When _treeb_preprocess_array_keys performed initial array preprocessing, it
 ...
 * TreebArrayKeyInfo info array, from input scan key references (to the keys in
 ...
 * this will leave this new treebrescan without any arrays at all.  This isn't
 ...
 */
static void
_treeb_preprocess_array_keys_final(IndexScanDesc scan, int *keyDataMap)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	Relation	rel = scan->indexRelation;
	int			arrayidx = 0;
	...
	/*
	 * Nothing for us to do when _treeb_preprocess_array_keys only had to deal
	...
	for (int output_ikey = 0; output_ikey < so->numberOfKeys; output_ikey++)
	{
		...
		if (outkey->sk_strategy != TreebEqualStrategyNumber)
			continue;
		...
		if (!(outkey->sk_flags & SK_SEARCHARRAY))
		{
			 ...
			 */
			if (!(outkey->sk_flags & SK_TREEB_REQFWD))
				continue;
			...
			_treeb_setup_array_cmp(scan, outkey, elemtype,
		...
		{
			TreebArrayKeyInfo *array = &so->arrayKeys[arrayidx];
	...
	if (scan->parallel_scan && so->numArrayKeys > INDEX_MAX_KEYS)
		ereport(ERROR,
				(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
				 errmsg_internal("number of array scan keys left by preprocessing (%d) exceeds the maximum allowed by parallel treeb index scans (%d)",
								 so->numArrayKeys, INDEX_MAX_KEYS)));
}

/*
 * _treeb_setup_array_cmp() -- Set up array comparison functions
 *
 ...
 * See _treeb_advance_array_keys for an explanation of why it'll need to treat
 ...
 */
static void
_treeb_setup_array_cmp(IndexScanDesc scan, ScanKey skey, Oid elemtype,
					FmgrInfo *orderproc, FmgrInfo **sortprocp)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	Relation	rel = scan->indexRelation;
	...
	Assert(skey->sk_strategy == TreebEqualStrategyNumber);
	...
	if (elemtype == opcintype)
	{
		...
		*orderproc = *index_getprocinfo(rel, skey->sk_attno, TREEBORDER_PROC);
		if (sortprocp)
			*sortprocp = orderproc;
	 ...
	 * incomplete, but only in cases where it's quite likely that _treeb_first
	 * would fail in just the same way (had we not failed before it could).
	 */
	cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1],
								 opcintype, elemtype, TREEBORDER_PROC);
	if (!RegProcedureIsValid(cmp_proc))
		elog(ERROR, "missing support function %d(%u,%u) for attribute %d of index \"%s\"",
			 TREEBORDER_PROC, opcintype, elemtype, skey->sk_attno,
	 ...
	 */
	cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1],
								 elemtype, elemtype, TREEBORDER_PROC);
	if (!RegProcedureIsValid(cmp_proc))
		elog(ERROR, "missing support function %d(%u,%u) for attribute %d of index \"%s\"",
			 TREEBORDER_PROC, elemtype, elemtype,
...
/*
 * _treeb_find_extreme_element() -- get least or greatest array element
 *
 * scan and skey identify the index column, whose opfamily determines the
 * comparison semantics.  strat should be TreebLessStrategyNumber to get the
 * least element, or TreebGreaterStrategyNumber to get the greatest.
 */
static Datum
_treeb_find_extreme_element(IndexScanDesc scan, ScanKey skey, Oid elemtype,
...
/*
 * _treeb_sort_array_elements() -- sort and de-dup array elements
 *
 ...
 */
static int
_treeb_sort_array_elements(ScanKey skey, FmgrInfo *sortproc, bool reverse,
						Datum *elems, int nelems)
{
	TreebSortArrayContext cxt;
	...
	qsort_arg(elems, nelems, sizeof(Datum),
			  _treeb_compare_array_elements, &cxt);
...
/*
 * _treeb_merge_arrays() -- merge next array's elements into an original array
 *
 ...
 * _treeb_sort_array_elements.  sortproc is the same-type ORDER proc that was
 ...
 */
static bool
_treeb_merge_arrays(IndexScanDesc scan, ScanKey skey, FmgrInfo *sortproc,
				 ...
				 Datum *elems_next, int nelems_next)
{
	...
	if (origelemtype != nextelemtype)
	{
		 ...
		 */
		cmp_proc = get_opfamily_proc(rel->rd_opfamily[skey->sk_attno - 1],
									 origelemtype, nextelemtype, TREEBORDER_PROC);
		if (!RegProcedureIsValid(cmp_proc))
		{
	...
	for (int i = 0, j = 0; i < nelems_orig_start && j < nelems_next;)
	{
		Datum	   *oelem = elems_orig + i,
				   *nelem = elems_next + j;
		int			res = _treeb_compare_array_elements(oelem, nelem, &cxt);
 ...
 */
static bool
_treeb_compare_array_scankey_args(IndexScanDesc scan, ScanKey arraysk, ScanKey skey,
							   FmgrInfo *orderproc, TreebArrayKeyInfo *array,
							   bool *qual_ok)
{
	Relation	rel = scan->indexRelation;
	...
	Assert((arraysk->sk_flags & SK_SEARCHARRAY) &&
		   arraysk->sk_strategy == TreebEqualStrategyNumber);
	Assert(!(skey->sk_flags & (SK_ISNULL | SK_ROW_HEADER | SK_ROW_MEMBER)));
	Assert(!(skey->sk_flags & SK_SEARCHARRAY) ||
		   skey->sk_strategy != TreebEqualStrategyNumber);

	/*
	 * _treeb_binsrch_array_skey searches an array for the entry best matching a
	 ...
	 * _treeb_binsrch_array_skey applies the correct comparator.
	 *
	...
	if (skey->sk_subtype != opcintype && skey->sk_subtype != InvalidOid)
	{
		 ...
		 *
		 * Scalar scan key's argument will be passed to _treeb_compare_array_skey
	...
	matchelem = _treeb_binsrch_array_skey(orderprocp, false,
 ...
 */
static int
_treeb_compare_array_elements(const void *a, const void *b, void *arg)
{
	Datum		da = *((const Datum *) a);
	Datum		db = *((const Datum *) b);
	TreebSortArrayContext *cxt = (TreebSortArrayContext *) arg;
	int32		compare;
...
/*
 * _treeb_compare_array_skey() -- apply array comparison function
 *
 ...
 *			>0 if tupdatum > arrdatum.
 *
 * This is essentially the same interface as _treeb_compare: both functions
 * compare the value that they're searching for to a binary search pivot.
 * However, unlike _treeb_compare, this function's "tuple argument" comes first,
 * while its "array/scankey argument" comes second.
*/
static inline int32
_treeb_compare_array_skey(FmgrInfo *orderproc,
					   Datum tupdatum, bool tupnull,
					   Datum arrdatum, ScanKey cur)
{
	int32		result = 0;

	Assert(cur->sk_strategy == TreebEqualStrategyNumber);

	if (tupnull)				/* NULL tupdatum */
	{
		if (cur->sk_flags & SK_ISNULL)
			result = 0;			/* NULL "=" NULL */
		else if (cur->sk_flags & SK_TREEB_NULLS_FIRST)
	...
	{
		if (cur->sk_flags & SK_TREEB_NULLS_FIRST)
	...
	{
		/*
		 * Like _treeb_compare, we need to be careful of cross-type comparisons,
		 ...
		 *
		 * _treeb_compare does it the wrong way around (flip when *ASC*) in order
...
/*
 * _treeb_binsrch_array_skey() -- Binary search for next matching array key
 *
 ...
 *
 * Also sets *set_elem_result to the result _treeb_compare_array_skey returned
 ...
 */
static int
_treeb_binsrch_array_skey(FmgrInfo *orderproc,
					   bool cur_elem_trig, ScanDirection dir,
					   Datum tupdatum, bool tupnull,
					   TreebArrayKeyInfo *array, ScanKey cur,
					   int32 *set_elem_result)
{
	...
	if (cur_elem_trig)
	{
		...
		Assert(cur->sk_flags & SK_TREEB_REQFWD);
		 ...
		 * (_treeb_checkkeys must have called _treeb_tuple_before_array_skeys, which
		...
		if (ScanDirectionIsForward(dir))
		{
			...
			if (high_elem >= low_elem)
			{
				...
				result = _treeb_compare_array_skey(orderproc, tupdatum, tupnull,
												arrdatum, cur);
...
/*
 * _treeb_start_array_keys() -- Initialize array keys at start of a scan
 *
 * Set up the cur_elem counters and fill in the first sk_argument value for
 * each array scankey.
 */
void
_treeb_start_array_keys(IndexScanDesc scan, ScanDirection dir)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	int			i;
	...
	{
		TreebArrayKeyInfo *curArrayKey = &so->arrayKeys[i];
...
/*
 * _treeb_advance_array_keys_increment() -- Advance to next set of array elements
 *
 ...
 */
static bool
_treeb_advance_array_keys_increment(IndexScanDesc scan, ScanDirection dir)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	 ...
	 * end after treebgettuple returns "false".)
	 *
...
/*
 * _treeb_rewind_nonrequired_arrays() -- Rewind non-required arrays
 *
 * Called when _treeb_advance_array_keys decides to start a new primitive index
 * scan on the basis of the current scan position being before the position
 * that _treeb_first is capable of repositioning the scan to by applying an
 ...
 * first descent of the tree by _treeb_first.  Later on, there could also be a
 * second descent, that places the scan right before tuples >= "(200, 3, 5)".
 * _treeb_first must never be allowed to build an insertion scan key whose "c"
 ...
 * Not all callers are at risk of _treeb_first using a non-required array like
 ...
 *
 * Note: _bt_verify_arrays_treeb_first is called by an assertion to enforce that
 * everybody got this right.
 */
static void
_treeb_rewind_nonrequired_arrays(IndexScanDesc scan, ScanDirection dir)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	int			arrayidx = 0;
	...
	for (int ikey = 0; ikey < so->numberOfKeys; ikey++)
	{
		...
		TreebArrayKeyInfo *array = NULL;
		int			first_elem_dir;

		if (!(cur->sk_flags & SK_SEARCHARRAY) ||
			cur->sk_strategy != TreebEqualStrategyNumber)
			continue;

		array = &so->arrayKeys[arrayidx++];
		Assert(array->scan_key == ikey);

		if ((cur->sk_flags & (SK_TREEB_REQFWD | SK_TREEB_REQBKWD)))
			continue;
...
/*
 * _treeb_tuple_before_array_skeys() -- too early to advance required arrays?
 *
 ...
 * scan's current _treeb_readpage-wise tuple.
 *
 * readpagetup callers must only call here when _treeb_check_compare already set
 * continuescan=false.  We help these callers deal with _treeb_check_compare's
 ...
 * a _treeb_check_compare-set sktrig value that indicates which scan key
 ...
 * that were already deemed to have been satisfied inside _treeb_check_compare.
 *
 ...
 * inequality strategy scan key wasn't satisfied in _treeb_check_compare when we
 * return true.  In fact, we automatically return false when passed such an
 * inequality sktrig by readpagetup callers -- _treeb_check_compare's initial
 ...
 */
static bool
_treeb_tuple_before_array_skeys(IndexScanDesc scan, ScanDirection dir,
							 IndexTuple tuple, TupleDesc tupdesc, int tupnatts,
							 bool readpagetup, int sktrig, bool *scanBehind)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	...
	for (int ikey = sktrig; ikey < so->numberOfKeys; ikey++)
	{
		 ...
		 * _treeb_advance_array_keys caller requires that we track *scanBehind
		 * without concern for scan direction.
		 */
		if ((cur->sk_flags & (SK_TREEB_REQFWD | SK_TREEB_REQBKWD)) == 0)
		{
			Assert(!readpagetup);
		...
		/*
		 * Deal with inequality strategy scan keys that _treeb_check_compare set
		 * continuescan=false for
		 */
		if (cur->sk_strategy != TreebEqualStrategyNumber)
		{
			/*
			 * When _treeb_check_compare indicated that a required inequality
			 * scan key wasn't satisfied, there's no need to verify anything;
			 * caller always calls _treeb_advance_array_keys with this sktrig.
			 */
			if (readpagetup)
				return false;
		...
		result = _treeb_compare_array_skey(&so->orderProcs[ikey],
										tupdatum, tupnull,
										cur->sk_argument, cur);
...
/*
 * _treeb_start_prim_scan() -- start scheduled primitive index scan?
 *
 * Returns true if _treeb_checkkeys scheduled another primitive index scan, just
 ...
 * after _treeb_first or _treeb_next return false.
 */
bool
_treeb_start_prim_scan(IndexScanDesc scan, ScanDirection dir)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	...
	/*
	 * Array keys are advanced within _treeb_checkkeys when the scan reaches the
	 ...
	 *
	 * _treeb_checkkeys sets a simple flag variable to schedule another primitive
	 * index scan.  The flag tells us what to do.
	 *
	 * We cannot rely on _treeb_first always reaching _treeb_checkkeys.  There are
	 * various cases where that won't happen.  For example, if the index is
	 * completely empty, then _treeb_first won't call _treeb_readpage/_treeb_checkkeys.
	 * We also don't expect a call to _treeb_checkkeys during searches for a
	 ...
	 * It's up to code under the control of _treeb_first to always set the flag
	 ...
	 * leaf root page).  If _treeb_checkkeys cannot be reached by a primitive
	 ...
	 * cases, but that's the correct behavior (even _treeb_advance_array_keys
	 * won't always advance the arrays at the point they become "exhausted").
	 */
	if (so->needPrimScan)
	{
		Assert(_bt_verify_arrays_treeb_first(scan, dir));

		/*
		 * Flag was set -- must call _treeb_first again, which will reset the
...
/*
 * _treeb_advance_array_keys() -- Advance array elements using a tuple
 *
 ...
 * All later _treeb_check_compare calls also use the same new qual that was first
 * used here (at least until the next call here advances the keys once again).
 * It's convenient to structure _treeb_check_compare rechecks of caller's tuple
 * (using the new qual) as one the steps of advancing the scan's array keys,
 * so this function works as a wrapper around _treeb_check_compare.
 *
 * Like _treeb_check_compare, we'll set pstate.continuescan on behalf of the
 * caller, and return a boolean indicating if caller's tuple satisfies the
 * scan's new qual.  But unlike _treeb_check_compare, we set so->needPrimScan
 ...
 *
 * Caller must use _treeb_tuple_before_array_skeys to determine if the current
 ...
 * of stopping the scan).  No call to _treeb_tuple_before_array_skeys is required
 ...
 * do this for inequalities that _treeb_check_compare set continuescan=false for.
 * They'll advance the array keys here, just like any other scan key that
 * _treeb_check_compare stops on.  (This can even happen _after_ we advance the
 * array keys, in which case we'll advance the array keys a second time.  That
 * way _treeb_checkkeys caller always has its required arrays advance to the
 * maximum possible extent that its tuple will allow.)
 */
static bool
_treeb_advance_array_keys(IndexScanDesc scan, TREEBReadPageState *pstate,
					   IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
					   int sktrig, bool sktrig_required)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	...
	if (sktrig_required)
	{
		 ...
		 */
		Assert(!_treeb_tuple_before_array_skeys(scan, dir, tuple, tupdesc,
	...
	Assert(_treeb_verify_keys_with_arraykeys(scan));
	...
	for (int ikey = 0; ikey < so->numberOfKeys; ikey++)
	{
		...
		else
		{
			 ...
			 */
			if (((ScanDirectionIsForward(dir) &&
				  (cur->sk_flags & (SK_TREEB_REQBKWD))) ||
				 (ScanDirectionIsBackward(dir) &&
				  (cur->sk_flags & (SK_TREEB_REQFWD)))))
		...
		if (cur->sk_flags & (SK_TREEB_REQFWD | SK_TREEB_REQBKWD))
		{
			...
			if (cur->sk_attno > tupnatts)
			{
				/* Set this just like _treeb_tuple_before_array_skeys */
				Assert(sktrig < ikey);
				so->scanBehind = true;
			}
		}
		 ...
		 * _treeb_check_compare indicated triggered array advancement, if any.
		 *
		 ...
		 * optimization; _treeb_tuple_before_array_skeys already confirmed that
		 ...
		 * on _treeb_check_compare like this; we aren't capable of directly
		 ...
		 * wasn't the one that _treeb_check_compare stopped on, though.
		 *
		 * Note: if our later call to _treeb_check_compare (to recheck caller's
		 ...
		 */
		else if (cur->sk_strategy != TreebEqualStrategyNumber)
			continue;
		...
		if (array)
		{
			 ...
			 */
			set_elem = _treeb_binsrch_array_skey(&so->orderProcs[ikey],
											  cur_elem_trig, dir,
		 ...
		 * this array's set_elem" by calling _treeb_advance_array_keys_increment.
		 ...
		 * be eliminated by _treeb_preprocess_keys.  It won't matter if some of
	 ...
	 */
	if (beyond_end_advance && !_treeb_advance_array_keys_increment(scan, dir))
		goto end_toplevel_scan;

	Assert(_treeb_verify_keys_with_arraykeys(scan));

	/*
	 * Does tuple now satisfy our new qual?  Recheck with _treeb_check_compare.
	 *
	 ...
	 * will still require a recheck call to _treeb_check_compare.  They'll still
	 ...
	 * _treeb_check_compare call that used the old/original qual.)
	 *
	 ...
	 * satisfies the new qual, so it's safe to just skip the _treeb_check_compare
		...
		(!sktrig_required && all_satisfied))
	{
		...
		/* Recheck _treeb_check_compare on behalf of caller */
		if (_treeb_check_compare(scan, dir, tuple, tupnatts, tupdesc,
							  false, false, false,
		 ...
		 *
		 * It's possible that our _treeb_check_compare call indicated that the
		 ...
		 *
		 * We make a strong, general guarantee that every _treeb_checkkeys call
		...
		if (unlikely(!continuescan))
		{
			...
			Assert(sktrig_required);
			Assert(so->keyData[nsktrig].sk_strategy != TreebEqualStrategyNumber);
			...
			satisfied = _treeb_advance_array_keys(scan, pstate, tuple, tupnatts,
	 ...
	 * for _treeb_check_compare to behave as if they are required in the current
	 * scan direction to deal with NULLs.  We'll account for that separately.)
	 */
	Assert(_treeb_tuple_before_array_skeys(scan, dir, tuple, tupdesc, tupnatts,
										false, 0, NULL) ==
	 ...
	 * it'll be set during this call to _treeb_tuple_before_array_skeys.  Either
	 * way, it'll be set correctly (for the whole page) after this point.
	 */
	if (!all_required_satisfied && pstate->finaltup &&
		_treeb_tuple_before_array_skeys(scan, dir, pstate->finaltup, tupdesc,
									 TreebTupleGetNAtts(pstate->finaltup, rel),
	 ...
	 *
	 * There is a chance that _treeb_checkkeys (which checks so->scanBehind) will
	...
	if (so->scanBehind && has_required_opposite_direction_only)
	{
		 ...
		 *
		 * _treeb_check_compare simply won't stop the scan for a scan key that's
		 ...
		 * scan will allow _treeb_first to skip ahead by a great many leaf pages
	 ...
	 * _treeb_first could skip straight to.  Groveling through all those pages
	 * will always give correct answers, but it can be very inefficient.  We
	 * must avoid needlessly scanning extra pages.
	 *
	 * Separately, it's possible that _treeb_check_compare set continuescan=false
	 * for a scan key that's required in the opposite direction only.  This is
	 * a special case, that happens only when _treeb_check_compare sees that the
	 ...
	 * (_treeb_first is expected to skip over the group of NULLs by applying a
	 ...
	 * also before the _treeb_first-wise start of tuples for our new qual.  That
	...
	{
		int			nfinaltupatts = TreebTupleGetNAtts(pstate->finaltup, rel);
		 ...
		 * cannot reuse work from caller's earlier _treeb_check_compare call.
		 *
		 * Flip the scan direction when calling _treeb_check_compare this time,
		...
		opsktrig = 0;
		flipped = -dir;
		_treeb_check_compare(scan, flipped,
		...
		if (unlikely(!continuescanflip &&
					 so->keyData[opsktrig].sk_strategy != TreebEqualStrategyNumber))
		{
			/*
			 * It's possible for the same inequality to be unsatisfied by both
			 * caller's tuple (in scan's direction) and finaltup (in the
			 * opposite direction) due to _treeb_check_compare's behavior with
			 * NULLs
			 */
			 ...
			 */
			_treeb_rewind_nonrequired_arrays(scan, dir);
	 ...
	 * search of the page, implemented by calling _treeb_check_compare and then
	 * _treeb_tuple_before_array_skeys for each tuple.
	 *
	 ...
	 */
	pstate->continuescan = true;	/* Override _treeb_check_compare */
	so->needPrimScan = false;	/* _treeb_readpage has more tuples to check */
...
new_prim_scan:
	 ...
	 */
	pstate->continuescan = false;	/* Tell _treeb_readpage we're done... */
	so->needPrimScan = true;	/* ...but call _treeb_first again */

	if (scan->parallel_scan)
		_treeb_parallel_primscan_schedule(scan, pstate->prev_scan_page);
...
end_toplevel_scan:
	 ...
	 */
	pstate->continuescan = false;	/* Tell _treeb_readpage we're done... */
	so->needPrimScan = false;	/* ...don't call _treeb_first again, though */
...
/*
 *	_treeb_preprocess_keys() -- Preprocess scan keys
 *
 ...
 * order as corresponding entries from the scan's TreebArrayKeyInfo array info.
 *
 * The output keys are marked with flags SK_TREEB_REQFWD and/or SK_TREEB_REQBKWD
 * if they must be satisfied in order to continue the scan forward or backward
 * respectively.  _treeb_checkkeys uses these flags.  For example, if the quals
 * are "x = 1 AND y < 4 AND z < 5", then _treeb_checkkeys will reject a tuple
 * (1,2,7), but we must continue the scan in case there are tuples (1,3,z).
 * But once we reach tuples like (1,4,z) we can stop scanning because no
 * later tuples could match.  This is reflected by marking the x and y keys,
 * but not the z key, with SK_TREEB_REQFWD.  In general, the keys for leading
 * attributes with "=" keys are marked both SK_TREEB_REQFWD and SK_TREEB_REQBKWD.
 * For the first attribute without an "=" key, any "<" and "<=" keys are
 * marked SK_TREEB_REQFWD while any ">" and ">=" keys are marked SK_TREEB_REQBKWD.
 ...
 * _treeb_first will arbitrarily pick one of the keys to do the initial
 ...
 */
void
_treeb_preprocess_keys(IndexScanDesc scan)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	int			numberOfKeys = scan->numberOfKeys;
	...
	ScanKey		outkeys;
	ScanKey		cur;
	TreebScanKeyPreproc xform[TreebMaxStrategyNumber];
	bool		test_result;
	int			i,
				j;
	AttrNumber	attno;
	...
	{
		/*
		 * Only need to do preprocessing once per treebrescan, at most.  All
	...
	arrayKeyData = _treeb_preprocess_array_keys(scan);
	if (!so->qual_ok)
	{
	 ...
	 * as our input if _treeb_preprocess_array_keys just allocated it, else just
	 * use scan->keyData[]
	 */
	if (arrayKeyData)
	{
	...
	if (cur->sk_attno < 1)
		elog(ERROR, "treeb index keys must be ordered by attribute");

	/* We can short-circuit most of the work if there's just one key */
	if (numberOfKeys == 1)
	{
		/* Apply indoption to scankey (might change sk_strategy!) */
		if (!_treeb_fix_scankey_strategy(cur, indoption))
			so->qual_ok = false;
		memcpy(outkeys, cur, sizeof(ScanKeyData));
		so->numberOfKeys = 1;
		/* We can mark the qual as required if it's for first index col */
		if (cur->sk_attno == 1)
			_treeb_mark_scankey_required(outkeys);
		if (arrayKeyData)
		{
			/*
			 * Don't call _treeb_preprocess_array_keys_final in this fast path
	...
	for (i = 0;; cur++, i++)
	{
		...
		if (i == numberOfKeys || cur->sk_attno != attno)
		{
			 ...
			 */
			if (xform[TreebEqualStrategyNumber - 1].skey)
			{
				ScanKey		eq = xform[TreebEqualStrategyNumber - 1].skey;
				TreebArrayKeyInfo *array = NULL;
				...
				if (arrayKeyData && (eq->sk_flags & SK_SEARCHARRAY))
				{
					...
					eq_in_ikey = xform[TreebEqualStrategyNumber - 1].ikey;
					eq_arrayidx = xform[TreebEqualStrategyNumber - 1].arrayidx;
				...
				for (j = TreebMaxStrategyNumber; --j >= 0;)
				{
					ScanKey		chk = xform[j].skey;

					if (!chk || j == (TreebEqualStrategyNumber - 1))
						continue;
					...
					if (_treeb_compare_scankey_args(scan, chk, eq, chk,
												 array, orderproc,
												 &test_result))
					{
						if (!test_result)
			...
			if (xform[TreebLessStrategyNumber - 1].skey
				&& xform[TreebLessEqualStrategyNumber - 1].skey)
			{
				ScanKey		lt = xform[TreebLessStrategyNumber - 1].skey;
				ScanKey		le = xform[TreebLessEqualStrategyNumber - 1].skey;

				if (_treeb_compare_scankey_args(scan, le, lt, le, NULL, NULL,
											 &test_result))
				{
					if (test_result)
						xform[TreebLessEqualStrategyNumber - 1].skey = NULL;
					else
						xform[TreebLessStrategyNumber - 1].skey = NULL;
				}
			}

			/* try to keep only one of >, >= */
			if (xform[TreebGreaterStrategyNumber - 1].skey
				&& xform[TreebGreaterEqualStrategyNumber - 1].skey)
			{
				ScanKey		gt = xform[TreebGreaterStrategyNumber - 1].skey;
				ScanKey		ge = xform[TreebGreaterEqualStrategyNumber - 1].skey;

				if (_treeb_compare_scankey_args(scan, ge, gt, ge, NULL, NULL,
											 &test_result))
				{
					if (test_result)
						xform[TreebGreaterEqualStrategyNumber - 1].skey = NULL;
					else
						xform[TreebGreaterStrategyNumber - 1].skey = NULL;
				}
			}
			...
			for (j = TreebMaxStrategyNumber; --j >= 0;)
			{
				if (xform[j].skey)
				{
					...
					if (priorNumberOfEqualCols == attno - 1)
						_treeb_mark_scankey_required(outkey);
				}
			}
		...
		if (cur->sk_flags & SK_ROW_HEADER)
		{
			 ...
			 */
			Assert(j != (TreebEqualStrategyNumber - 1));
			continue;
		}
		...
		{
			/* _treeb_preprocess_array_keys marked this array key redundant */
			Assert(arrayKeyData);
			Assert(cur->sk_flags & SK_SEARCHARRAY);
			continue;
		}
		...
		{
			/* _treeb_preprocess_array_keys kept this array key */
			Assert(arrayKeyData);
			arrayidx++;
		}
		...
		else
		{
			 ...
			 * if possible
			 */
			if (j == (TreebEqualStrategyNumber - 1) && arrayKeyData)
			{
				/*
				 ...
				 * matter, since _treeb_compare_scankey_args is aware that two
				 * SEARCHARRAY scan keys mean that _treeb_preprocess_array_keys
				 * failed to eliminate redundant arrays through array merging.
				 * _treeb_compare_scankey_args just returns false when it sees
				 * this; it won't even try to examine either array.
				 */
			}

			if (_treeb_compare_scankey_args(scan, cur, cur, xform[j].skey,
										 array, orderproc, &test_result))
			{
				...
				if (test_result)
				{
					 ...
					 */
					if (j != (TreebEqualStrategyNumber - 1) ||
					...
					else
					{
						 ...
						 * scan key.  _treeb_compare_scankey_args expects us to
						 * always keep arrays (and discard non-arrays).
						 */
						Assert(!(cur->sk_flags & SK_SEARCHARRAY));
					}
				}
				else if (j == (TreebEqualStrategyNumber - 1))
				{
					/* key == a && key == b, but a != b */
					so->qual_ok = false;
					return;
			...
			else
			{
				 ...
				 * even with incomplete opfamilies.  _treeb_advance_array_keys
				 * depends on this.
				 */
				ScanKey		outkey = &outkeys[new_numberOfKeys++];
	 ...
	 */
	if (arrayKeyData)
		_treeb_preprocess_array_keys_final(scan, keyDataMap);
 ...
 * _treeb_start_prim_scan is about to start a just-scheduled new primitive scan.
 *
 ...
 * See _treeb_rewind_nonrequired_arrays comments for an explanation.
 */
static bool
_bt_verify_arrays_treeb_first(IndexScanDesc scan, ScanDirection dir)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	int			arrayidx = 0;
	...
	for (int ikey = 0; ikey < so->numberOfKeys; ikey++)
	{
		...
		if (((cur->sk_flags & SK_TREEB_REQFWD) && ScanDirectionIsForward(dir)) ||
			((cur->sk_flags & SK_TREEB_REQBKWD) && ScanDirectionIsBackward(dir)))
			continue;
	...
	return _treeb_verify_keys_with_arraykeys(scan);
}
 ...
 */
static bool
_treeb_verify_keys_with_arraykeys(IndexScanDesc scan)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
 ...
 * opfamily (_treeb_preprocess_array_keys will merge array keys together to make
 * sure of that).
 *
 ...
 */
static bool
_treeb_compare_scankey_args(IndexScanDesc scan, ScanKey op,
						 ScanKey leftarg, ScanKey rightarg,
						 TreebArrayKeyInfo *array, FmgrInfo *orderproc,
						 bool *result)
{
	...
	if ((leftarg->sk_flags | rightarg->sk_flags) & SK_ISNULL)
	{
		...
		if (op->sk_flags & SK_TREEB_NULLS_FIRST)
			strat = TreebCommuteStrategyNumber(strat);
	...
	if (array)
	{
		...
		leftarray = ((leftarg->sk_flags & SK_SEARCHARRAY) &&
					 leftarg->sk_strategy == TreebEqualStrategyNumber);
		rightarray = ((rightarg->sk_flags & SK_SEARCHARRAY) &&
					  rightarg->sk_strategy == TreebEqualStrategyNumber);

		/*
		 * _treeb_preprocess_array_keys is responsible for merging together array
		 ...
		 */
		if (leftarray)
			return _treeb_compare_array_scankey_args(scan, leftarg, rightarg,
												  orderproc, array, result);
		else if (rightarray)
			return _treeb_compare_array_scankey_args(scan, rightarg, leftarg,
												  orderproc, array, result);
	 ...
	 * operators.)
	 *
	 * If the sk_strategy was flipped by _treeb_fix_scankey_strategy, we have to
	 * un-flip it to get the correct opfamily member.
	 */
	strat = op->sk_strategy;
	if (op->sk_flags & SK_TREEB_DESC)
		strat = TreebCommuteStrategyNumber(strat);
 ...
 * NULL comparison value.  Since all treeb operators are assumed strict,
 ...
 */
static bool
_treeb_fix_scankey_strategy(ScanKey skey, int16 *indoption)
{
	int			addflags;

	addflags = indoption[skey->sk_attno - 1] << SK_TREEB_INDOPTION_SHIFT;

	/*
	 * We treat all treeb operators as strict (even if they're not so marked
	 ...
	 * particular strategy in this case, so set it to TreebEqualStrategyNumber
	...
	if (skey->sk_flags & SK_ISNULL)
	{
		...
		{
			if (skey->sk_flags & SK_TREEB_NULLS_FIRST)
				skey->sk_strategy = TreebGreaterStrategyNumber;
			else
				skey->sk_strategy = TreebLessStrategyNumber;
			skey->sk_subtype = InvalidOid;
			skey->sk_collation = InvalidOid;
		}
		else
	...
	if ((addflags & SK_TREEB_DESC) && !(skey->sk_flags & SK_TREEB_DESC))
		skey->sk_strategy = TreebCommuteStrategyNumber(skey->sk_strategy);
	skey->sk_flags |= addflags;
	...
	if (skey->sk_flags & SK_ROW_HEADER)
	{
		...
		for (;;)
		{
			...
			addflags = indoption[subkey->sk_attno - 1] << SK_TREEB_INDOPTION_SHIFT;
			if ((addflags & SK_TREEB_DESC) && !(subkey->sk_flags & SK_TREEB_DESC))
				subkey->sk_strategy = TreebCommuteStrategyNumber(subkey->sk_strategy);
 ...
 */
static void
_treeb_mark_scankey_required(ScanKey skey)
{
	int			addflags;
	...
	switch (skey->sk_strategy)
	{
		case TreebLessStrategyNumber:
		case TreebLessEqualStrategyNumber:
			addflags = SK_TREEB_REQFWD;
			break;
		case TreebEqualStrategyNumber:
			addflags = SK_TREEB_REQFWD | SK_TREEB_REQBKWD;
			break;
		case TreebGreaterEqualStrategyNumber:
		case TreebGreaterStrategyNumber:
			addflags = SK_TREEB_REQBKWD;
			break;
		default:
			elog(ERROR, "unrecognized StrategyNumber: %d",
				 (int) skey->sk_strategy);
 ...
 * _treeb_preprocess_keys(), above, about how this is done.
 *
 ...
 */
bool
_treeb_checkkeys(IndexScanDesc scan, TREEBReadPageState *pstate, bool arrayKeys,
			  IndexTuple tuple, int tupnatts)
{
	...
	Assert(TreebTupleGetNAtts(tuple, scan->indexRelation) == tupnatts);

	res = _treeb_check_compare(scan, dir, tuple, tupnatts, tupdesc,
	...
	if (pstate->prechecked || pstate->firstmatch)
	{
		 ...
		 */
		Assert(res == _treeb_check_compare(scan, dir, tuple, tupnatts, tupdesc,
										false, false, false,
										&dcontinuescan, &dikey));
	...
	/*
	 * Only one _treeb_check_compare call is required in the common case where
	 * there are no equality strategy array scan keys.  Otherwise we can only
	 * accept _treeb_check_compare's answer unreservedly when it didn't set
	...
	/*
	 * _treeb_check_compare call set continuescan=false in the presence of
	 ...
	 */
	if (_treeb_tuple_before_array_skeys(scan, dir, tuple, tupdesc, tupnatts, true,
									 ikey, NULL))
	{
		/*
		 ...
		 *
		 * _treeb_advance_array_keys occasionally sets so->scanBehind to signal
		 ...
		 */
		Assert(!so->scanBehind ||
			   so->keyData[ikey].sk_strategy == TreebEqualStrategyNumber);
		if (unlikely(so->scanBehind) && pstate->finaltup &&
			_treeb_tuple_before_array_skeys(scan, dir, pstate->finaltup, tupdesc,
										 TreebTupleGetNAtts(pstate->finaltup,
															scan->indexRelation),
										 false, 0, NULL))
		{
		...
		{
			/* Override _treeb_check_compare, continue primitive scan */
			pstate->continuescan = true;
			...
			if (pstate->rechecks >= LOOK_AHEAD_REQUIRED_RECHECKS)
			{
				...
				_treeb_checkkeys_look_ahead(scan, pstate, tupnatts, tupdesc);

				/*
				 * Might have set pstate.skip to a later page offset.  When
				 * that happens then _treeb_readpage caller will inexpensively
	 ...
	 * the scan.
	 */
	return _treeb_advance_array_keys(scan, pstate, tuple, tupnatts, tupdesc,
								  ikey, true);
}
 ...
 * array keys).
 *
 * This is a subroutine for _treeb_checkkeys.  We provisionally assume that
 ...
 * This allows _treeb_advance_array_keys caller to avoid infinite recursion.
 */
static bool
_treeb_check_compare(IndexScanDesc scan, ScanDirection dir,
				  IndexTuple tuple, int tupnatts, TupleDesc tupdesc,
				  bool advancenonrequired, bool prechecked, bool firstmatch,
				  bool *continuescan, int *ikey)
{
	...
	*continuescan = true;		/* default assumption */
	...
	for (; *ikey < so->numberOfKeys; (*ikey)++)
	{
		 ...
		 */
		if (((key->sk_flags & SK_TREEB_REQFWD) && ScanDirectionIsForward(dir)) ||
			((key->sk_flags & SK_TREEB_REQBKWD) && ScanDirectionIsBackward(dir)))
			requiredSameDir = true;
		else if (((key->sk_flags & SK_TREEB_REQFWD) && ScanDirectionIsBackward(dir)) ||
				 ((key->sk_flags & SK_TREEB_REQBKWD) && ScanDirectionIsForward(dir)))
		...
		if (key->sk_attno > tupnatts)
		{
			 ...
			 */
			Assert(TreebTupleIsPivot(tuple));
			continue;
		}
		...
		{
			if (_treeb_check_rowcompare(key, tuple, tupnatts, tupdesc, dir,
									 continuescan))
				continue;
			return false;
		}
		...
		if (isNull)
		{
			if (key->sk_flags & SK_TREEB_NULLS_FIRST)
			{
				/*
				 ...
				 * (_treeb_advance_array_keys also relies on this behavior during
				 * forward scans.)
				 */
				if ((key->sk_flags & (SK_TREEB_REQFWD | SK_TREEB_REQBKWD)) &&
					ScanDirectionIsBackward(dir))
					*continuescan = false;
			}
			else
											...
											datum, key->sk_argument)))
		{
			 ...
			 * initial positioning in _treeb_first() when they are available. See
			 * comments in _treeb_first().
			 */
			if (requiredSameDir)
				*continuescan = false;
			 ...
			 */
					 ...
					 key->sk_strategy == TreebEqualStrategyNumber &&
					 (key->sk_flags & SK_SEARCHARRAY))
				return _treeb_advance_array_keys(scan, NULL, tuple, tupnatts,
											  tupdesc, *ikey, false);
 ...
 * to pass the qual.
 *
 * This is a subroutine for _treeb_checkkeys/_treeb_check_compare.
 */
static bool
_treeb_check_rowcompare(ScanKey skey, IndexTuple tuple, int tupnatts,
					 TupleDesc tupdesc, ScanDirection dir, bool *continuescan)
{
	...
	for (;;)
	{
		...
		if (isNull)
		{
			if (subkey->sk_flags & SK_TREEB_NULLS_FIRST)
			{
				/*
				 * Since NULLs are sorted before non-NULLs, we know we have
				 * reached the lower limit of the range of values for this
				 ...
				 * forward scans.)
				 */
				if ((subkey->sk_flags & (SK_TREEB_REQFWD | SK_TREEB_REQBKWD)) &&
					ScanDirectionIsBackward(dir))
					*continuescan = false;
			}
			else
		...
		if (subkey->sk_flags & SK_ISNULL)
		{
			...
			if (subkey != (ScanKey) DatumGetPointer(skey->sk_argument))
				subkey--;
			if ((subkey->sk_flags & SK_TREEB_REQFWD) &&
				ScanDirectionIsForward(dir))
				*continuescan = false;
			else if ((subkey->sk_flags & SK_TREEB_REQBKWD) &&
					 ScanDirectionIsBackward(dir))
				*continuescan = false;
			return false;
		}
		...
		if (subkey->sk_flags & SK_TREEB_DESC)
 ...
 *
 * This is a subroutine for _treeb_checkkeys.  Called when _treeb_readpage's linear
 ...
 * instructs _treeb_readpage to skip ahead to that tuple next (could be past the
 ...
 */
static void
_treeb_checkkeys_look_ahead(IndexScanDesc scan, TREEBReadPageState *pstate,
						 int tupnatts, TupleDesc tupdesc)
{
	ScanDirection dir = pstate->dir;
	 ...
	 * allows _treeb_readpage to skip over more tuples
	 */
	...
	if (_treeb_tuple_before_array_skeys(scan, dir, ahead, tupdesc, tupnatts,
									 false, 0, NULL))
	{
		/*
		 * Success -- instruct _treeb_readpage to skip ahead to very next tuple
...
/*
 * _treeb_killitems - set LP_DEAD state for items an indexscan caller has
 * told us were killed
 *
 ...
 */
void
_treeb_killitems(IndexScanDesc scan)
{
	TreebScanOpaque so = (TreebScanOpaque) scan->opaque;
	Page		page;
	...
	if (TreebScanPosIsPinned(so->currPos))
	{
		 ...
		 */
		droppedpin = false;
		_treeb_lockbuf(scan->indexRelation, so->currPos.buf, TREEB_READ);
	...
	else
	{
		...
		buf = _treeb_getbuf(scan->indexRelation, so->currPos.currPage, TREEB_READ);
		...
		else
		{
			...
			_treeb_relbuf(scan->indexRelation, buf);
			return;
		}
	}
	...
	for (i = 0; i < numKilled; i++)
	{
		...
		TreebScanPosItem *kitem = &so->currPos.items[itemIndex];
		...
		while (offnum <= maxoff)
		{
			...
			if (TreebTupleIsPosting(ituple))
			{
				int			pi = i + 1;
				int			nposting = TreebTupleGetNPosting(ituple);
				int			j;
				...
				{
					ItemPointer item = TreebTupleGetPostingN(ituple, j);
	 ...
	 * TREEBP_HAS_GARBAGE flag, which is likewise just a hint.  (Note that we
 ...
 * assignment of "vacuum cycle IDs" to currently-active treeb vacuuming
 ...
 *
 * Access to the shared memory area is controlled by TreebVacuumLock.
...
typedef struct TreebOneVacInfo
{
	LockRelId	relid;			/* global identifier of an index */
	TreebCycleId	cycleid;		/* cycle ID for its active VACUUM */
} TreebOneVacInfo;

typedef struct TreebVacInfo
{
	TreebCycleId	cycle_ctr;		/* cycle ID most recently assigned */
...
/*
 * _treeb_vacuum_cycleid --- get the active vacuum cycle ID for an index,
 ...
 */
TreebCycleId
_treeb_vacuum_cycleid(Relation rel)
{
	TreebCycleId	result = 0;
	int			i;

	/* Share lock is enough since this is a read-only operation */
	LWLockAcquire(TreebVacuumLock, LW_SHARED);

	for (i = 0; i < treebvacinfo->num_vacuums; i++)
	{
		TreebOneVacInfo *vac = &treebvacinfo->vacuums[i];
	...
	LWLockRelease(TreebVacuumLock);
	return result;
}

/*
 * _treeb_start_vacuum --- assign a cycle ID to a just-starting VACUUM operation
 *
 * Note: the caller must guarantee that it will eventually call
 * _treeb_end_vacuum, else we'll permanently leak an array slot.  To ensure
 ...
 */
TreebCycleId
_treeb_start_vacuum(Relation rel)
{
	TreebCycleId	result;
	int			i;
	TreebOneVacInfo *vac;

	LWLockAcquire(TreebVacuumLock, LW_EXCLUSIVE);

	/*
	 * Assign the next cycle ID, being careful to avoid zero as well as the
	 * reserved high values.
	 */
	result = ++(treebvacinfo->cycle_ctr);
	if (result == 0 || result > MAX_TREEB_CYCLE_ID)
		result = treebvacinfo->cycle_ctr = 1;

	/* Let's just make sure there's no entry already for this index */
	for (i = 0; i < treebvacinfo->num_vacuums; i++)
	{
		vac = &treebvacinfo->vacuums[i];
		if (vac->relid.relId == rel->rd_lockInfo.lockRelId.relId &&
			vac->relid.dbId == rel->rd_lockInfo.lockRelId.dbId)
		{
			/*
			 ...
			 * we expect _treeb_end_vacuum() to be called before transaction
	...
	/* OK, add an entry */
	if (treebvacinfo->num_vacuums >= treebvacinfo->max_vacuums)
	{
		LWLockRelease(TreebVacuumLock);
		elog(ERROR, "out of treebvacinfo slots");
	}
	vac = &treebvacinfo->vacuums[treebvacinfo->num_vacuums];
	vac->relid = rel->rd_lockInfo.lockRelId;
	vac->cycleid = result;
	treebvacinfo->num_vacuums++;
...
/*
 * _treeb_end_vacuum --- mark a treeb VACUUM operation as done
 *
 ...
 */
void
_treeb_end_vacuum(Relation rel)
{
	int			i;
	...
	for (i = 0; i < treebvacinfo->num_vacuums; i++)
	{
			...
			vac->relid.dbId == rel->rd_lockInfo.lockRelId.dbId)
		{
			...
			*vac = treebvacinfo->vacuums[treebvacinfo->num_vacuums - 1];
			treebvacinfo->num_vacuums--;
			break;
		}
	}
...
/*
 * _treeb_end_vacuum wrapped as an on_shmem_exit callback function
 */
void
_treeb_end_vacuum_callback(int code, Datum arg)
{
	_treeb_end_vacuum((Relation) DatumGetPointer(arg));
}

/*
 * TreebShmemSize --- report amount of shared memory space needed
 */
Size
TreebShmemSize(void)
{
	...
	size = offsetof(TreebVacInfo, vacuums);
	size = add_size(size, mul_size(MaxBackends, sizeof(TreebOneVacInfo)));
	return size;
}

/*
 * TreebShmemInit --- initialize this module's shared memory
 */
void
TreebShmemInit(void)
{
	...
	treebvacinfo = (TreebVacInfo *) ShmemInitStruct("Treeb Vacuum State",
											  TreebShmemSize(),
											  &found);
	...
	if (!IsUnderPostmaster)
	{
		 ...
		 */
		treebvacinfo->cycle_ctr = (TreebCycleId) time(NULL);

		treebvacinfo->num_vacuums = 0;
		treebvacinfo->max_vacuums = MaxBackends;
	}
	else
		Assert(found);
}
...
bytea *
treeboptions(Datum reloptions, bool validate)
{
	static const relopt_parse_elt tab[] = {
		{"fillfactor", RELOPT_TYPE_INT, offsetof(TreebOptions, fillfactor)},
		{"vacuum_cleanup_index_scale_factor", RELOPT_TYPE_REAL,
		offsetof(TreebOptions, vacuum_cleanup_index_scale_factor)},
		{"deduplicate_items", RELOPT_TYPE_BOOL,
		offsetof(TreebOptions, deduplicate_items)}
	};
...
/*
 *	treebproperty() -- Check boolean properties of indexes.
 *
 * This is optional, but handling AMPROP_RETURNABLE here saves opening the rel
 * to call treebcanreturn.
 */
bool
treebproperty(Oid index_oid, int attno,
		   ...
		   bool *res, bool *isnull)
{
	switch (prop)
	{
		case AMPROP_RETURNABLE:
			...
			if (attno == 0)
				return false;
			/* otherwise, treeb can always return data */
			*res = true;
			return true;
...
/*
 *	treebbuildphasename() -- Return name of index build phase.
 */
char *
treebbuildphasename(int64 phasenum)
{
	switch (phasenum)
	{
		case PROGRESS_CREATEIDX_SUBPHASE_INITIALIZE:
			return "initializing";
		case PROGRESS_TREEB_PHASE_INDEXBUILD_TABLESCAN:
			return "scanning table";
		case PROGRESS_TREEB_PHASE_PERFORMSORT_1:
			return "sorting live tuples";
		case PROGRESS_TREEB_PHASE_PERFORMSORT_2:
			return "sorting dead tuples";
		case PROGRESS_TREEB_PHASE_LEAF_LOAD:
...
/*
 *	_treeb_truncate() -- create tuple without unneeded suffix attributes.
 *
 ...
 */
IndexTuple
_treeb_truncate(Relation rel, IndexTuple lastleft, IndexTuple firstright,
			 TreebScanInsert itup_key)
{
	 ...
	 */
	Assert(!TreebTupleIsPivot(lastleft) && !TreebTupleIsPivot(firstright));

	/* Determine how many attributes must be kept in truncated tuple */
	keepnatts = _treeb_keep_natts(rel, lastleft, firstright, itup_key);
	...
	if (TreebTupleIsPosting(pivot))
	{
		...
		pivot->t_info |= MAXALIGN(TreebTupleGetPostingOffset(firstright));
	}
	...
	{
		TreebTupleSetNAtts(pivot, keepnatts, false);
		return pivot;
	}
	 ...
	 * split from the left side.  treeb conceptualizes this case as an
	...
	TreebTupleSetNAtts(tidpivot, nkeyatts, true);
	pivotheaptid = TreebTupleGetHeapTID(tidpivot);
	 ...
	 * legally usable.)
	 */
	ItemPointerCopy(TreebTupleGetMaxHeapTID(lastleft), pivotheaptid);
	...
	Assert(ItemPointerCompare(TreebTupleGetMaxHeapTID(lastleft),
							  TreebTupleGetHeapTID(firstright)) < 0);
	Assert(ItemPointerCompare(pivotheaptid,
							  TreebTupleGetHeapTID(lastleft)) >= 0);
	Assert(ItemPointerCompare(pivotheaptid,
							  TreebTupleGetHeapTID(firstright)) < 0);
#else
	 ...
	 */
	ItemPointerCopy(TreebTupleGetHeapTID(firstright), pivotheaptid);
...
/*
 * _treeb_keep_natts - how many key attributes to keep when truncating.
 *
 ...
 */
static int
_treeb_keep_natts(Relation rel, IndexTuple lastleft, IndexTuple firstright,
			   TreebScanInsert itup_key)
{
	...
	/*
	 * _treeb_compare() treats truncated key attributes as having the value minus
	...
	/*
	 * Assert that _treeb_keep_natts_fast() agrees with us in passing.  This is
	 * expected in an allequalimage index.
	 */
	Assert(!itup_key->allequalimage ||
		   keepnatts == _treeb_keep_natts_fast(rel, lastleft, firstright));

	return keepnatts;
}

/*
 * _treeb_keep_natts_fast - fast bitwise variant of _treeb_keep_natts.
 *
 ...
 * save cycles.
 *
 * The approach taken here usually provides the same answer as _treeb_keep_natts
 * will (for the same pair of tuples from a heapkeyspace index), since the
 * majority of treeb opclasses can never indicate that two datums are equal
 ...
 * _treeb_keep_natts would.
 *
 * Callers can rely on the fact that attributes considered equal here are
 * definitely also equal according to _treeb_keep_natts, even when the index uses
 * an opclass or collation that is not "allequalimage"/deduplication-safe.
 * This weaker guarantee is good enough for treebsplitloc.c caller, since false
 * negatives generally only have the effect of making leaf page splits use a
 * more balanced split point.
 */
int
_treeb_keep_natts_fast(Relation rel, IndexTuple lastleft, IndexTuple firstright)
{
	TupleDesc	itupdesc = RelationGetDescr(rel);
...
/*
 *  _treeb_check_natts() -- Verify tuple has expected number of attributes.
 *
 ...
 * sanity check.
 *
 * Testing a tuple directly with TreebTupleGetNAtts() should generally be
 ...
 * what is expected on each version of treeb.
 */
bool
_treeb_check_natts(Relation rel, bool heapkeyspace, Page page, OffsetNumber offnum)
{
	...
	tupnatts = TreebTupleGetNAtts(itup, rel);

	/* !heapkeyspace indexes do not support deduplication */
	if (!heapkeyspace && TreebTupleIsPosting(itup))
		return false;
	...
	if (natts != nkeyatts && TreebTupleIsPosting(itup))
		return false;
	 ...
	 */
	if (TreebTupleGetHeapTID(itup) != NULL && tupnatts != nkeyatts)
		return false;
...
/*
 *
 *  _treeb_check_third_page() -- check whether tuple fits on a treeb page at all.
 *
 ...
 */
void
_treeb_check_third_page(Relation rel, Relation heap, bool needheaptidspace,
					 Page page, IndexTuple newtup)
{
	...
	if (itemsz <= TreebMaxItemSize(page))
		return;
	 ...
	 */
	if (!needheaptidspace && itemsz <= TreebMaxItemSizeNoHeapTid(page))
		return;
	...
	ereport(ERROR,
			(errcode(ERRCODE_PROGRAM_LIMIT_EXCEEDED),
			 errmsg("index row size %zu exceeds treeb version %u maximum %zu for index \"%s\"",
					itemsz,
					needheaptidspace ? TREEB_VERSION : TREEB_NOVAC_VERSION,
					needheaptidspace ? TreebMaxItemSize(page) :
					TreebMaxItemSizeNoHeapTid(page),
					RelationGetRelationName(rel)),
			 errdetail("Index row references tuple (%u,%u) in relation \"%s\".",
					   ItemPointerGetBlockNumber(TreebTupleGetHeapTID(newtup)),
					   ItemPointerGetOffsetNumber(TreebTupleGetHeapTID(newtup)),
 ...
 *
 * We use each attribute's TreebEQUALIMAGE_PROC opclass procedure.  If any
 * opclass either lacks a TreebEQUALIMAGE_PROC procedure or returns false, we
 ...
 */
bool
_treeb_allequalimage(Relation rel, bool debugmessage)
{
	...
	for (int i = 0; i < IndexRelationGetNumberOfKeyAttributes(rel); i++)
	{
		...
		equalimageproc = get_opfamily_proc(opfamily, opcintype, opcintype,
										   TreebEQUALIMAGE_PROC);

		/*
		 * If there is no TreebEQUALIMAGE_PROC then deduplication is assumed to
